...
def _extract_llm_text(attrs: dict[str, Any]) -> tuple[str, str]:
    ...
    Real OpenLLMetry / traceloop instrumentation emits this attribute as a
    JSON-encoded *string*, not a nested OTLP array — `_maybe_decode_json`
    parses that string back into a list while passing an already-decoded
