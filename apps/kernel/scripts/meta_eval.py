...
def _make_client(base_url: str | None):
    ...
    bit twice during the 2026-05-08 W5.5 local-meta-eval attempt.
    Cloud route (``base_url is None``) keeps the SDK's normal env-var
    discovery so ``ANTHROPIC_API_KEY`` works as before.
    """
