...
async def test_approve_advances_state(
    ...
    resp = await api_client.post(
        ...
        json={"decided_by": "human:jit", "comment": "ship it"},
    )
    assert resp.status_code == 200
    body = resp.json()
    ...
    assert body["approval"]["decided_by"] == "human:jit"
...
async def test_approve_invalid_approver_type(
    ...
    resp = await api_client.post(
        f"/api/proposals/{seeded['proposal_id']}/approve",
        json={"decided_by": "human:jit", "approver_type": "robot"},
    )
    assert resp.status_code == 422
