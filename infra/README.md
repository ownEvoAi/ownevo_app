...
`infra/litellm/ollama.yaml` — proxy config for hybrid NL-gen runs: Anthropic `/v1/messages` → Ollama `/api/chat` translation, with passthrough entries for cloud models. Used by `apps/kernel/scripts/run_a4_4_local_smoke.sh`. Set `OWNEVO_OLLAMA_HOST` to the Ollama daemon host (default `192.168.1.50`).
