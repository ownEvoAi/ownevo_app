...
### Added
- **Published container images on GHCR — self-host without building from source.** A new `publish-images` workflow builds the kernel and web images on every `v*` tag push and pushes them to the GitHub Container Registry as `ghcr.io/ownevoai/ownevo-kernel` and `ghcr.io/ownevoai/ownevo-web`, tagged with the full version, the `major.minor` and `major` series, and `latest`. It authenticates with the built-in `GITHUB_TOKEN` (no registry secret to configure) and caches layers across runs via the GitHub Actions cache backend. `docker-compose.yml` now reads the image references from `OWNEVO_KERNEL_IMAGE` / `OWNEVO_WEB_IMAGE` (defaulting to the local build tags), so the same compose file runs either built-from-source or pulled: `docker compose pull && docker compose up -d --no-build`. The one-shot `migrate` service shares the kernel image, so a published tag covers migrations too. Images are `linux/amd64` (they run under Docker's emulation on arm64; a native arm64 build is deferred because emulating the kernel's ML stack under QEMU is prohibitively slow in CI). New pull-and-run section in `docs/DEPLOYMENT.md`; self-hosting is covered by the Business Source License's Additional Use Grant (everything except offering ownEvo as a hosted competing service).
- **Job-queue observability — depth gauges, a read endpoint, a Jobs tab, and a structured failure signal.** Makes the durable job queue visible end to end. `/metrics` gains an `ownevo_jobs{status="queued|running|failed"}` gauge family (the `failed` series always present, so an alert rule can key on it even at zero); because `jobs` is under row-level security, the unscoped scrape aggregates per workspace via the same enumerate-and-bind pattern the reaper uses, and any aggregation error omits the block rather than 500-ing the scrape. A new read-only `GET /api/jobs` lists the requesting workspace's recent jobs (bounded `limit`, max 200; optional `status` filter → 422 on an unknown value) plus workspace-wide `counts` per status so a caller sees queue depth without a second request; the raw job payload is never exposed (only `workflow_id` is lifted out). The web app gains a read-only **Jobs** tab under Records showing the depth counts and recent jobs (kind, status, attempts, workflow, last error). When a job exhausts its retries the worker now logs a structured `ERROR` carrying `job_failed_terminal` / `job_id` / `workflow_id` / `kind` / `attempts` / `last_error` so a log- or Sentry-based alert keys on a field instead of regex-matching the message (the `ownevo_jobs{status="failed"}` gauge is the Prometheus signal). Job retention/pruning is intentionally out of scope — `succeeded`/`failed` rows accumulate like traces do, so the `failed` gauge is cumulative.
- **Durable job queue so trigger-fired iterations survive a kernel restart.** When a cron/threshold/Slack/email trigger fired the `run_iteration` action it spawned the iteration as a detached in-process task: a kernel restart (deploy, crash, OOM, SIGTERM) killed the task, the startup reaper correctly failed the orphaned `iterations` row, but the requested work was simply lost — never retried. The trigger action now inserts a row on a new `jobs` table (migration `0040_jobs_queue.sql`) *before* the work is dispatched, and a background `JobWorker` (mirroring the existing trigger scheduler / cluster auto-trigger lifecycle) claims it with `FOR UPDATE SKIP LOCKED`, heartbeats while it runs, and re-queues any job whose worker died mid-run (stale heartbeat) — so the work resumes on the next boot instead of vanishing. Failures retry with exponential backoff up to `max_attempts` (default 3) and then go terminally `failed`, so a poison job cannot wedge the worker. Enqueue is idempotent per workflow via a partial unique index on active (queued/running) jobs, preserving the one-iteration-at-a-time intent the manual "Run iteration" button already enforces. The `jobs` table is workspace-scoped under `FORCE ROW LEVEL SECURITY` like every domain table, so the worker enumerates the global `workspaces` index and binds each workspace GUC before draining it, same as the reaper. The HTTP "Run iteration" button is unchanged (it blocks on the request, so its work is user-retryable); only the trigger path — which has no client to retry — is made durable. The worker follows the trigger scheduler by default (`OWNEVO_TRIGGER_SCHEDULER`), with `OWNEVO_JOB_WORKER=true|false` to override; both off by default. The `iterations` row stays the run artifact a job produces — a re-queued job runs a fresh iteration rather than resuming a partial one, since the mid-cycle LLM outputs were never persisted (matching the reaper's existing semantics).
- **Keyset pagination on the trace-list endpoints.** `GET /api/traces` and `GET /api/workflows/{id}/traces` now accept `limit` (default and max 500) and an opaque `cursor` query param, and return a `next_cursor` field (null on the last page) for walking to the next/older page on the `(started_at DESC, id DESC)` ordering. This bounds reads that were previously a hazard: the per-workflow list had **no limit at all** (it loaded every row for the workflow), and the workspace-wide list silently truncated at a hard 500 with no way to page past it. The default page size preserves the prior 500-row cap, but truncation is now explicit — a full page returns a `next_cursor` instead of dropping the rest. A malformed cursor is a 400 rather than a silent reset to the first page. Table partitioning and a retention policy on the high-volume `traces` JSONB table remain Phase-2 work.
- **Graceful-shutdown drain (`OWNEVO_SHUTDOWN_DRAIN_SECONDS`).** Optional drain window inserted between SIGTERM/SIGINT arrival and uvicorn's own shutdown sequence. When the env var is a positive number, the handler marks the app as shutting down (so `/readyz` returns 503 and the load balancer pulls the instance) and keeps the socket open for that many seconds so in-flight and just-routed requests can complete. After the window elapses the original uvicorn signal handler is re-delivered so its normal graceful shutdown runs. Default `0` disables it entirely — no handler is installed and `make api` / Ctrl-C behaviour is unchanged.
- **Slow-query log (`OWNEVO_DB_SLOW_QUERY_MS`).** Registers an asyncpg query logger via the pool `init` callback (once per physical connection) that emits a `WARNING` with structured `elapsed_ms` / `threshold_ms` / `query` fields when a query's server round-trip exceeds the threshold. Argument values are never logged (only the parameterised `$1`/`$2` query text, normalised and truncated to 500 chars). Default `1000` ms; `0` disables the logger so the per-query timing wrapper is never entered.
- **`/api/livez`, `/api/readyz`, and `/metrics` — readiness/liveness split + Prometheus gauges.** Third slice of operational hardening. `/api/livez` is dependency-free (no DB) and always returns 200 so a transient DB outage cannot trigger a restart loop. `/api/readyz` returns 200 when the pool answers `SELECT 1` and **503** when it cannot, so a load balancer can drain the instance rather than routing requests that will fail. `/metrics` exposes `ownevo_up`, `ownevo_uptime_seconds`, `ownevo_db_up`, `ownevo_db_pool_size` / `_idle` / `_in_use`, and `ownevo_sandbox_max_concurrent` in Prometheus text exposition format (`text/plain; version=0.0.4`) without pulling in `prometheus_client`. Pool gauges are omitted (not zeroed) when no pool is attached so a scraper can tell "pool absent" from "pool empty". Process uptime is stamped at import via `time.monotonic()` so `/metrics` works even when the lifespan is bypassed. All three endpoints are unauthenticated, consistent with the existing `/api/health`, which is kept unchanged for back-compat. `render_metrics` is a pure function so the exposition format is unit-testable without a running app or DB.
- **Sentry error reporting (opt-in via `SENTRY_DSN`).** Second slice of operational hardening, building on the global exception handler + request-id middleware + JSON logs that landed earlier. When `SENTRY_DSN` is set, `init_sentry` (called first thing in the FastAPI lifespan, before any other boot work) wires the SDK with the FastAPI + Starlette integrations so every uncaught route exception is shipped alongside the in-process structured log line the global handler already writes. The request-id middleware now stamps a `request_id` Sentry tag per request, so a Sentry event and a JSON log line for the same incident share the correlation key the client already saw in the `X-Request-Id` response header — one customer-side `error_id` resolves to one Sentry event and one log entry. A `before_send` hook drops the captured request body and cookies before send because the kernel's POST surface accepts workflow specs, agent payloads, and provider credentials; PII (cookies, client IPs) is also disabled via `send_default_pii=False`. Performance traces default to `OWNEVO_SENTRY_TRACES_SAMPLE_RATE=0.0` so a noisy deployment cannot silently burn the Sentry quota — operators opt in by raising the rate (validated against `[0.0, 1.0]`; a malformed value fails the boot rather than silently disabling traces). `OWNEVO_ENV` is reused for the environment tag and `OWNEVO_SENTRY_RELEASE` lets the deployer pin a release identifier (unset → Sentry's VCS auto-detect). With no DSN configured, `init_sentry` is a no-op — dev and CI installs do not need to set an opt-out, and `tag_request` stays cheap (the SDK's default client silently drops scope mutations).
- **Pool sizing and per-connection statement_timeout are env-tunable.** The kernel's asyncpg pool now reads three new env vars at boot: `OWNEVO_DB_POOL_MIN_SIZE` (default 1), `OWNEVO_DB_POOL_MAX_SIZE` (default 10), and `OWNEVO_DB_STATEMENT_TIMEOUT_MS` (default 30000). The timeout is applied via the pool's `setup` callback on every acquire because asyncpg's pool runs `RESET ALL` on release, which clears any `SET` issued earlier; the callback re-applies the GUC after the reset so the cap holds for every checkout, not just the first one. A query that exceeds the cap is cancelled by Postgres and surfaces as `asyncpg.QueryCanceledError`, so one runaway statement can no longer pin a connection until the upstream client gives up. Setting `OWNEVO_DB_STATEMENT_TIMEOUT_MS=0` opts a process out (useful for long-running maintenance scripts that share the pool); env values are validated at boot so a typo fails loudly there rather than at the first query. The API lifespan now opens its pool through the same `open_pool()` helper that scripts already use, so production and dev paths see identical defaults instead of diverging hard-coded values.
- **Startup reaper for iteration rows orphaned by kernel restarts.** An iteration run is driven by the request task: phase 1 inserts a `'running'` row, phase 2 spends 30-90 s on LLM calls without a held connection, and phase 3 writes the terminal state. A kernel crash or restart between phases cancels the task and leaves the row in `'running'` forever, and the workflow's one-iteration-at-a-time guard then rejects every subsequent run on that workflow with HTTP 409 until an operator hand-edits the row. The FastAPI lifespan now runs a sweep before the API accepts traffic: any iteration still in `'running'` is closed as `'sandbox-error'` with `ended_at = now()` and an `iteration-reaped` audit entry recording the workflow id, iteration index, original `started_at`, and a `reason: 'orphaned-by-restart'` payload — so the action is visible in the hash-chained audit log and exportable. The sweep iterates the global `workspaces` index and binds the workspace GUC per tenant via `acquire_workspace_conn`, so soft-deleted workspaces are skipped (the orphan stays unreachable along with the rest of that tenant's rows) and a failing tenant is logged and skipped rather than blocking startup. New audit_kind value via migration `0037_audit_iteration_reaped.sql`. Resuming the partial run is not feasible — the LLM completions and mid-cycle outcomes were never persisted — so failing the row matches what the runner does on any other exception path.
- **Global exception handler, request-id middleware, and opt-in JSON logging.** First slice of operational hardening — gives the kernel a structured error path and a correlation key per request so production incidents are actually grepable. The new `RequestIdMiddleware` mints a 32-char hex id per request (or adopts a sanitised inbound `X-Request-Id` header — alphanumerics, dashes, underscores, up to 128 chars, so a hostile caller cannot inject header or log content), stows it on `request.state.request_id`, and echoes it back on every response. A new `install_exception_handler` registers a global catch for uncaught `Exception` that converts the bare Starlette 500 into a JSON body `{error: 'internal_server_error', error_id, detail}` carrying the same id, and emits an ERROR log record with the full traceback plus structured `request_id` / `error_id` / `method` / `path` / `exc_class` fields. `HTTPException` and `RequestValidationError` are left to FastAPI's own handlers — they already return structured 4xx bodies. The opt-in `OWNEVO_LOG_FORMAT=json` flag installs a dependency-free `JsonFormatter` on the root logger that emits one JSON object per record with extras preserved (so the exception handler's structured fields land as top-level attributes for shippers like Datadog / Loki / Cloud Logging); unset, the stdlib defaults stay in place and `make api` looks the same as before. Idempotent — calling `configure_logging` twice does not stack handlers. The wider operational-hardening bundle (Sentry hookup, `/metrics`, readiness/liveness split, pool sizing + statement timeout) is intentionally out of scope here; this slice lands the cross-cutting plumbing the rest will hang off.
- **Workspace provisioning and active-workspace switcher.** Completes the auth rollout (step 4 of 4, see `docs/AUTH.md`). A brand-new authenticated user with no workspace membership is now gated by Next.js middleware and redirected to `/setup/new-workspace`, a create-workspace screen that calls a new kernel endpoint (`POST /api/internal/workspaces`, service-token authenticated) and updates the session in place via `unstable_update` without forcing a re-sign-in. The session JWT now also handles the `trigger='update'` case so workspace switches are reflected immediately. Users with multiple workspaces see a live switcher in the sidebar that calls `switchWorkspaceAction` — a server action that validates membership from the current JWT and redirects to the new workspace. New `auth.config.ts` (edge-safe, providers-free) split from `auth.ts` so the Next.js middleware can parse JWT sessions without pulling in provider code or `kernel-sync` imports into the Edge runtime. The provisioning endpoint and its token-auth tests round out the internal-service layer alongside the existing auth-sync endpoint.
- **Authentication at the web edge — sign-in plus per-request workspace resolution end to end.** The web app now authenticates users with Auth.js (NextAuth v5) and asserts the resolved principal to the kernel, completing the path the multi-tenant substrate and the kernel-side assertion verifier left as a stub. Two providers register from the same code: a dev/credentials provider (gated on `OWNEVO_DEV_AUTH=true`, logs in as the seeded local user with no external round-trip) and Google OIDC (gated on `AUTH_GOOGLE_ID` / `AUTH_GOOGLE_SECRET`), so local dev and tests need no Google credentials while a real deployment gets Google from the same wiring. The session is an encrypted JWT cookie (no server-side session table). On first sign-in the web app upserts the user + provider identity through a new kernel endpoint, `POST /api/internal/auth/sync` (authenticated by the shared `OWNEVO_INTERNAL_AUTH_KEY` as a service bearer token, since no workspace is bound yet), which writes the global non-RLS `users` / `user_identities` tables and returns the caller's live `workspace_members`. Every kernel call then carries a short-lived (120 s) signed `(user_id, workspace_id)` assertion minted in the web tier; the TS minter is byte-for-byte compatible with the kernel's Python `mint_workspace_assertion`, and the assertion is injected through a server-registered hook so the client-safe `lib/api` module never pulls in `next/headers`. Zero-config local dev still works: with no shared key set, the dev provider mirrors the kernel's seeded `dev-user`/`default` fallback. New env: `AUTH_SECRET` (encrypts the session cookie) and the optional `AUTH_GOOGLE_*` pair. Workspace provisioning UI + an active-workspace switcher are the remaining slice; see `docs/AUTH.md`.
- **Multi-tenant workspace substrate (non-enforcing step 1).** New `workspaces` table and a `workspace_id` column on every workspace-scoped domain table (`workflows`, `skills`, `skill_versions`, `skill_deployments`, `eval_cases`, `failure_clusters`, `traces`, `iterations`, `iteration_case_outputs`, `proposals`, `approvals`, `meta_evals`, `learnings`, `captured_sandbox_runs`, `receiver_tokens`, `integration_credentials`, `audit_entries`) via migration `0033`. Each column is `NOT NULL DEFAULT 'default'` with a foreign key to `workspaces(id)` and a supporting index; existing rows backfill to a seeded `default` workspace, so single-tenant behavior is unchanged. New `tenant_session.py` binds a connection to the active workspace by setting the `app.workspace_id` session GUC (`set_workspace` / `current_workspace`), and `get_conn` now sets it on every request (currently the `default` workspace; per-request resolution lands with the auth layer). Demo-mode rate-limiting tables are intentionally excluded — they are global state, not workspace data. Row-level security enforcement lands in the companion entry below (migration `0034`); the two-step split let the columns and read paths be verified against scoped data before the enforcement switch was flipped.
- **Multi-tenant row-level security enforcement (step 2).** Migration `0034` turns isolation on: every one of the 17 workspace-scoped tables gets `ENABLE` + `FORCE ROW LEVEL SECURITY` and a per-table policy scoping reads (`USING`) and writes (`WITH CHECK`) to the session workspace bound on the connection. `FORCE` is required because the kernel connects as the table owner, and a plain `ENABLE` leaves the owner exempt — so the production app role must be a non-superuser without `BYPASSRLS` (the isolation tests run under a dedicated such role to exercise the policies for real). The `workspace_id` column default changes from the literal `'default'` to `current_setting('app.workspace_id', true)`, so an insert auto-stamps the active workspace and an unscoped connection (GUC unset → NULL) fails closed against the `NOT NULL` column rather than silently writing to a default tenant. `tenant_session.py` gains `acquire_workspace_conn(pool, workspace_id)` for background workers and scripts that acquire connections directly (the request path already binds the GUC through `get_conn`); both binders refuse a missing or soft-deleted workspace. Workspace removal is **soft delete** (`workspaces.deleted_at`), not a cascade — after which `set_workspace` refuses to bind it, so the tenant's rows become unreachable while staying physically present (the append-only `audit_entries` cannot be row-deleted, and retention keeps the operation reversible). Three pre-conditions were resolved to make enforcement safe: background-worker `pool.acquire()` call sites now route through `acquire_workspace_conn`, the `integration_credentials` primary key widened to `(workspace_id, provider)`, and the `failure_clusters` fingerprint unique index is now scoped by `workspace_id`. Per-request workspace resolution remains a `default`-returning stub pending the auth layer, so single-tenant behavior is unchanged; dev/benchmark scripts that open their own connections must call `set_workspace(conn, DEFAULT_WORKSPACE_ID)` before touching scoped tables.
- **Auth schema substrate + design doc.** Groundwork for end-user authentication: migration `0035_auth_users.sql` adds `users`, `user_identities` (multi-provider), and `workspace_members` (carrying a `role` column the role-based approval work reuses). The three tables are **global and intentionally outside row-level security** — the request resolver reads membership before a workspace is bound, so scoping them by the GUC they help establish would be circular. Seeds a `dev-user` as `owner` of the `default` workspace so local flows keep working. `docs/AUTH.md` records the resolved design: authentication at the web edge (Auth.js — Google OIDC for cloud, a dev/credentials provider for local), identity propagated to the kernel as a short-lived HMAC-signed `Authorization: Bearer` assertion (no new JWT dependency; the kernel never sees Google tokens), a JWT-cookie web session, and explicit create/join-workspace on first login rather than an auto-provisioned tenant.
- **Per-request workspace resolution from a signed principal + membership gate.** Retires the `get_workspace_id` stub that hard-resolved every request to the `default` workspace — the step that turns the multi-tenant RLS substrate into real per-request tenancy. A request now resolves to a `Principal(user_id, workspace_id)` from a signed `Authorization: Bearer` assertion verified against `OWNEVO_INTERNAL_AUTH_KEY`, and `get_conn` checks `workspace_members` (a global / non-RLS table, readable while unbound) **before** binding the connection: a non-member or a soft-deleted workspace gets **403**, and because this is the only path to workspace-scoped data the gate can't be bypassed. `OWNEVO_DEV_AUTH=true` resolves an assertion-less request to the seeded dev principal in `default`; the fallback is refused unless the flag is explicitly true, so production fails closed (**401**) on unauthenticated requests. The kernel's membership re-check is defense-in-depth — the signed assertion is authoritative for `(user, workspace)` because only the web app holds the key and only signs membership-verified pairs.
- **Custom sign-in and error pages.** Auth.js's default unstyled sign-in and error screens are replaced with pages matching the app's design system. The sign-in page (`/auth/signin`) renders the dev-credentials email form when `OWNEVO_DEV_AUTH=true` and a "Sign in with Google" button when `AUTH_GOOGLE_ID`/`AUTH_GOOGLE_SECRET` are set, threads `callbackUrl` through every path, and shows inline errors for `CredentialsSignin` / `OAuthAccountNotLinked`. The error page (`/auth/error`) maps `AccessDenied`, `OAuthSignin`, `OAuthCallback`, `OAuthAccountNotLinked`, `Configuration`, and `Verification` to plain-language copy with a generic fallback. Middleware redirects unauthenticated requests to `/auth/signin` (not the bare Auth.js route) and allow-lists `/auth/*` so the sign-in page is always reachable.
- **Workspace invites and member management.** Link-based invites with no email infrastructure: an owner/admin mints an HMAC-signed token addressed to a `workspace_invites` row (migration `0036_workspace_invites.sql`), sends the URL through their own channel, and the invitee redeems it after signing in. The token carries a `kind` discriminator so it can't be replayed as a workspace assertion (or vice-versa), and the table is non-RLS because redemption happens before the redeemer is bound to the workspace. Three kernel endpoints — mint / redeem / revoke — gate on owner-or-admin membership; redeem is idempotent for the original redeemer and rejects a different account consuming the same URL. A new **Members** settings page lets an owner/admin see the roster (ordered owner → admin → member, creator anchored at the top), see outstanding invites and revoke them with one click, and mint a new invite whose URL is shown once with a Copy button — ownEvo never stores or transmits it. The accept page (`/invites/[token]`) is built around a viewer-relative status enum from a read-only preview endpoint (`pending`, `expired`, `revoked`, `redeemed_by_me`, `redeemed_by_other`, `email_mismatch`, `workspace_gone`), so every terminal state is surfaced up front with next-step copy instead of after a doomed click; visiting the page never burns the invite — only an explicit Accept does. Middleware allow-lists `/invites/*` so a brand-new user with no workspace can land on the page and redeem instead of being kicked to the create-workspace screen.
- **Process-global sandbox concurrency cap (`OWNEVO_SANDBOX_MAX_CONCURRENT`).** `LocalDockerSandbox.run()` is the only `docker run` chokepoint in the kernel and had no admission control: a caller fanning out N sandbox runs on one host (eval-case fan-out, an agent making many `run_pipeline` tool calls, concurrent workflow iterations) could spawn N simultaneous containers and thrash CPU/RAM. A module-level `asyncio.Semaphore` now caps concurrent runs (default 4, override via the env var); the slot is acquired before the temp dir is created and held for the entire container lifecycle (acquire → cleanup). The semaphore is a module singleton rather than a class attribute so a subclass can't silently shadow and bypass the cap, and it is lazily constructed on first use so it binds to the event loop that runs the first sandbox call.
- **Durable clustering — trigger and auto-clustering routed through the job queue.** The durable job queue made the trigger → `run_iteration` path survive a kernel restart, but clustering still ran inline in two background paths (the trigger scheduler's dispatch and the debounced auto-trigger), so a restart mid-run dropped that work with no retry and no `/api/jobs` visibility. A new `run_clustering` job kind (migration `0043`) routes both through the existing queue, mirroring `run_iteration`: the auto-trigger becomes a pure debounced enqueuer, and because the active-job dedup index is already keyed by `(workspace_id, kind, workflow_id)`, a clustering job and an iteration job for one workflow coexist while repeat enqueues collapse to one. Missing clustering/agent extras complete the job as a no-op instead of burning the retry budget on a condition retry can't fix. The manual path is unchanged — it already awaits the runner inline within the request, so a restart surfaces as a failed HTTP response rather than silently-dropped work.

### Changed
- **Trigger actions are now routed through workspace-scoped RLS connections.** All three trigger action functions (`action_run_clustering`, `action_run_iteration`, `action_ingest_failures`) now accept a `workspace_id` parameter and acquire connections via `acquire_workspace_conn` instead of bare `pool.acquire()`, so every workspace-scoped table write (`traces`, `failure_clusters`, `iterations`) is correctly bound under RLS. `TriggerDispatcher` resolves the workspace by querying the `workflows` row before dispatching; `EmailIngester` and `SlackIngester` fetch the workspace inline before calling `action_ingest_failures`. `action_run_iteration` no longer creates the `iterations` row upfront or returns an iteration ID — the runner owns that row's lifecycle. (It originally spawned the runner as a fire-and-forget background task; it now enqueues a durable `run_iteration` job instead — see the durable-job-queue entry under **Added**.)
- **The workflow render-layer concept is renamed from "primitive" to "view" everywhere.** The concept was named by audience — the operator-facing UI already said "View" while the code, the `/primitives` route, and the spec (`ui.tabs[].primitives[]`, `UIPrimitive`, the `ui-primitive` proposal kind) said "primitive", and the word even leaked into one form heading. Because the spec's `type` discriminator flows from the kernel into the web operate resolver's `switch`, a stray name silently renders nothing; collapsing to one canonical word removes that whole class of drift. The rename spans trace-format (`ui_primitives.py` → `ui_views.py`, `UIPrimitive`/`UIPrimitiveAdapter` → `UIView`/`UIViewAdapter`), the kernel spec field with a **`SCHEMA_VERSION` bump 1.4 → 1.5** (a structural rename, not additive — regenerated `workflow_spec.v1.5.json` snapshot), the kernel↔web API seam (route `/proposals/ui-primitive` → `/proposals/ui-view`, `proposal_kind` value `ui-primitive` → `ui-view`, payload key `primitives` → `views`, renamed atomically on both sides), and the web component/resolver/route/CSS layer (`/primitives` route → `/views`, `primitive-data-resolver.ts` → `view-data-resolver.ts`, `primitives.css` → `views.css`). Migration `0044` rewrites stored data in lockstep — `workflows.spec` (`ui.tabs[].primitives` → `views`, `schema_version` → 1.5) and `proposals.proposed_payload` JSONB, plus the in-place enum-value rename — which is **mandatory, not cosmetic**: the kernel re-parses stored specs through `extra="forbid"` models, so a stored `primitives` key would fail validation. The nine individual view *type* names (`MetricCards`, `TimeSeriesChart`, `TableView`, `AlertList`, `KanbanBoard`, `ScheduleGrid`, `ConversationView`, `SideBySideView`, `DocumentReader`) are unchanged, and `audit_entries` history plus the internal `operate_ui_primitives` design-agent dimension key (persisted in transcripts) are deliberately left untouched. The change also folds in a correctness fix to the propose-edit form, which previously offered a `HeadlineMetrics` type the kernel never emits (so a proposal made through it rendered blank) and omitted two supported views — the form now lists the full canonical nine under `KNOWN_VIEW_TYPES`.
- **Browser-side kernel calls now route through server actions.** Six client components called the kernel API directly from the browser with no user assertion — calls that fail auth once `OWNEVO_INTERNAL_AUTH_KEY` is set, and that the kernel's dev fallback only masks locally. Each is now wrapped in a `'use server'` action (the try-it run, the description / metric / sim / UI-view propose-edit flows, and the description-conflict scan) so the assertion is minted server-side before the fetch reaches the kernel.
- **Dev/benchmark scripts bind the workspace GUC through a single helper.** Under `FORCE ROW LEVEL SECURITY`, a connection that hasn't bound `app.workspace_id` reads zero rows from every scoped table and fails the NOT-NULL on insert — silently, with writes failing closed and reads returning empty. A new `connect_workspace_conn(db_url, workspace_id)` context manager (the single-connection analogue of `acquire_workspace_conn`) centralises connect-bind-close-on-exit, and the dev/benchmark scripts that open their own connections were swept onto it — or onto an explicit `set_workspace` bind — fixing several (`sim_replay_smoketest.py`, `tau3_inspect_task.py`, the M5/clustering/replay scripts) that were otherwise silently broken under enforced RLS.
- **CI now runs the DB-gated tests against a Postgres service container.** The `test` job runs against `pgvector/pgvector:pg16` with `OWNEVO_DATABASE_URL` set, so the ~32 tests that previously self-skipped without a database (migration, RLS-isolation, and query-level coverage) execute on every PR — closing the structural blind spot that let migration- and query-level regressions land green.

### Fixed
- **OTLP ingest route was non-functional under the production auth path.** Adding per-request workspace resolution to the OTLP ingest route collided with its existing receiver-token auth: both schemes read `Authorization: Bearer …`, but one expects a signed workspace identity assertion and the other a `receiver_tokens` row, and a single header can't be both. Worse, in production the receiver-token lookup hit a table that RLS had put under workspace scoping, so on an unbound connection it returned zero rows regardless of token validity — service-to-service trace push was broken. The route now derives the workspace from the token itself (`receiver_tokens.workspace_id`) rather than a web-app assertion (OTLP callers are service agents, not user sessions), and migration `0038_receiver_tokens_global.sql` makes `receiver_tokens` a global, non-RLS auth-gateway table since it must be queried before any workspace is bound (the `workspace_id` column stays as metadata and still auto-stamps on workspace-bound inserts).
- **Self-host bring-up: real migration runner, `db-migrate`/`db-reset`, and env secrets.** The `docker-compose.yml` Postgres service mounted `apps/kernel/migrations` into `docker-entrypoint-initdb.d`, which (a) only ran on a first-boot empty data volume so schema changes never applied on restart, (b) bypassed the `schema_migrations` ledger, and (c) broke on the `CREATE INDEX CONCURRENTLY` migrations (`0018b`/`0024`/`0027`) — psql wraps each initdb file in an implicit transaction that `CONCURRENTLY` refuses. The mount is removed; a one-shot `migrate` service now runs `scripts/migrate.py` (the real idempotent runner) before the kernel starts. The `infra/docker-compose.yml` (Postgres-only) likewise drops the mount. `Makefile` gains `db-migrate` (host-run idempotent apply against `OWNEVO_DATABASE_URL`) and `db-reset` (drop + recreate + migrate the dev database). `.env.example` now documents the two previously undocumented required secrets — `OWNEVO_CREDENTIALS_MASTER_KEY` (Fernet key for integration credentials at rest) and `OWNEVO_DEMO_SIGNING_KEY` (HS256 key for hosted-demo invite tokens) — each with a generation snippet. `docs/DEPLOYMENT.md` and `infra/README.md` are corrected to reflect that `make api` does not migrate (run `make db-migrate` first).
- **`complete_job` and `fail_job` now guard against stale-requeue races.** Both DB helpers previously matched `WHERE id = $1` only, so a late-finishing worker could silently overwrite a row that had already been re-queued by the stale-heartbeat detector and re-claimed by a second worker. Both WHERE clauses now also require `AND status = 'running'`, matching the guard that `heartbeat_job` already had. `fail_job` and `requeue_stale_jobs` also clear `claimed_at` and `heartbeat_at` (not just `claimed_by`) when returning a job to `queued`, so re-queued rows carry no stale ownership state.
- **Job-queue partial indexes now include `workspace_id` (migration `0041`).** `jobs_claim_idx` and `jobs_heartbeat_idx` from migration 0040 lacked `workspace_id` as the leading column. Under RLS, every claim and stale-scan query applies the workspace filter as a post-index condition, visiting all queued/running rows across all workspaces before narrowing to the active one. Migration 0041 drops and recreates both indexes with `workspace_id` leading so the planner can seek directly to the active workspace's rows.
- **Trace keyset-index migration (`0039`) could not be applied — missing `-- ownevo:no-txn` annotation.** The migration uses `CREATE INDEX CONCURRENTLY`, which Postgres refuses to run inside a transaction block, but it lacked the annotation both migration runners (`scripts/migrate.py` and the test-harness `migrate()`) detect to execute a file outside one. Applying it raised `ActiveSQLTransactionError`, which also broke every DB-backed test (the per-test fixture re-applies all migrations from scratch). The annotation is now present, matching every other `CONCURRENTLY` / `VALIDATE CONSTRAINT` migration in the directory.
- **Trigger tables' `workflow_id` foreign key could not be applied on a fresh database.** Migration `0032` declared `trigger_definitions.workflow_id`, `trigger_fires.workflow_id`, and `metric_samples.workflow_id` as `UUID`, but `workflows.id` is `text` (and the triggers code types `workflow_id` as `str`). A foreign key requires matching column types, so a clean `migrate` aborted with a datatype mismatch the moment it reached these tables. The three columns are now `text`, matching `workflows.id`; the `UUID` primary keys on the same tables are unchanged. The bug was masked in CI because the DB-backed tests skip when `OWNEVO_DATABASE_URL` is unset, and the schema-shape assertion in `test_db.py` had not yet been extended to cover the trigger tables and enums (now added).

### Security
- **Startup guards refuse dangerous auth-flag combinations.** The dev-auth fallback (`OWNEVO_DEV_AUTH=true`) resolves every unauthenticated request to a seeded dev user and the `default` workspace — intended for `make api` and tests, dangerous in production — and several misconfigurations previously surfaced only invisibly at runtime. Both the kernel (FastAPI lifespan) and the web app (Next.js `instrumentation.ts`) now fail loudly at boot: `OWNEVO_DEV_AUTH=true` together with `OWNEVO_INTERNAL_AUTH_KEY` (which would silently bypass workspace isolation) throws; `OWNEVO_DEV_AUTH=true` together with Google OAuth credentials (every Google sign-in would 500) throws; and with the explicit `OWNEVO_ENV=production` / `NODE_ENV=production` marker set, the process refuses to boot if dev-auth is on, the internal signing key is missing, or (kernel) the credentials master key is missing — so a prod-shaped misconfiguration produces an actionable startup error rather than silent insecure data access.
- **The Try-it endpoint bypassed the workspace membership check.** `POST /workflows/{id}/try` used a dependency that extracted the workspace from the signed assertion but skipped the `workspace_members` gate every other scoped route enforces. It now resolves the full principal and runs the same membership check (without holding a pooled connection during the LLM call), so a signed assertion for a workspace the user doesn't belong to can no longer reach the Try-it path.
- **Google accounts with unverified email can no longer be linked to an existing user.** Google issues OAuth tokens for accounts whose email address is unverified; without a guard, the kernel's email-match identity-linking path would associate such an identity with the existing owner of that address, handing an attacker their workspaces. A `signIn` callback now rejects (`AccessDenied`) any Google account where `email_verified` is not explicitly `true`. Relatedly, the `/api/internal/auth` route is hidden from the public OpenAPI schema and the auth-failure detail no longer leaks the signing-key env-var name.
- **Dev credentials provider is now mutually exclusive with `OWNEVO_INTERNAL_AUTH_KEY`.** Previously both could be set simultaneously (e.g. a staging environment with the service key configured but Google credentials not yet wired). Because the dev provider accepts any email with no password check and the kernel's `_upsert_user` links a new identity to any existing user with a matching email, co-deploying both flags opened an account-takeover path. `buildProviders()` now skips the dev credentials provider whenever `OWNEVO_INTERNAL_AUTH_KEY` is present, so the flags are mutually exclusive at runtime.
- **Concurrent first sign-ins for the same new email no longer return 500.** The `INSERT INTO users` in `_upsert_user` had no `ON CONFLICT` clause despite the table carrying `UNIQUE(email)`. Two simultaneous first sign-ins for the same brand-new email address both passed the existence check before either committed, then the second insert raised an unhandled `UniqueViolationError`. Changed to `INSERT ... ON CONFLICT (email) DO UPDATE ... RETURNING id`, making concurrent first sign-ins idempotent.

## [0.13.0] — 2026-05-25

### Added
- **MCP-client framework — a workflow's agent can read any connected system through an MCP server.** Rather than building bespoke per-source connectors, ownEvo consumes Model Context Protocol servers. New `apps/kernel/src/ownevo_kernel/mcp_client/` ships typed models, three auth-resolution paths (bearer / OAuth-refresh / service-principal), an `MCPTransport` Protocol with an official-SDK implementation, a registry that seals secrets at rest, and `MCPClient` (`list_tools` + `mcp_call`) with TTL session caching bounded by token expiry. Migration `0029_mcp_servers.sql` stores registered servers (endpoint, transport, auth kind/config, encrypted secret blob). `WorkflowSpec.DataSource` gains `kind="mcp"` + `mcp_server_id` (schema `1.1`→`1.2`, additive Literal union). The agent runtime advertises `mcp_list_tools` + `mcp_call` only when a run declares MCP sources, confined to the declared server ids; failed calls surface as `tool_call_result` errors and cluster like any other tool error. REST `/api/mcp/servers` (register / list / get / delete / test) behind an optional `mcp` extra. Hardening: `list_tools` / `call_tool` wrapped in a 30 s `asyncio.wait_for` so a stalled server can't pin a worker; endpoint and token URLs validated to http/https with a private-IP block (SSRF guard); a missing credentials key returns 503, not 500.
- **Slack, Google Workspace, and Microsoft 365 OAuth connectors.** An OAuth authorization-code flow in the kernel plus per-provider admin pages in the web app, built on the MCP-client framework. Migration `0030_mcp_oauth.sql` adds `mcp_oauth_clients` (per-provider app credentials, secret sealed at rest) and `mcp_oauth_states` (single-use CSRF nonce carrying the intended server name / scopes / endpoint across the consent round-trip). `mcp_client/providers.py` holds Slack / Google Workspace / Microsoft 365 presets (authorize + token URLs, default scopes + MCP endpoint, Google offline-refresh params, Microsoft tenant interpolation); `mcp_client/oauth.py` builds the consent URL and exchanges the code — a refresh token yields an auto-refreshing OAuth server, a long-lived token a bearer one. `/api/mcp/oauth` exposes the provider catalogue, client-credential CRUD, `start` (mint state → consent URL), and `callback` (exchange → register server → redirect back to settings). Web: three `settings/integrations/` provider pages sharing one form (register app credentials, run consent, manage connected servers) plus an integrations index and a Settings nav entry.
- **Spreadsheet and document uploads as workflow data sources.** A reviewer can upload data that lives in files rather than connected systems; ownEvo parses the file once and the agent reads the parsed form by id on every iteration (no re-upload). Migration `0031_data_uploads.sql` adds `data_uploads` (parsed schema + content + provenance + retention). New `data_ingest/` package: a spreadsheet parser (CSV / Excel / Parquet → columns + JSON-safe rows) and a document parser (PDF / DOCX → text + title / sections / tables; DOCX gets real structure, PDF best-effort), behind a lazy-imported optional `data-ingest` extra (pandas, openpyxl, pypdf, python-docx) so the core stays pandas-free. `/api/uploads` handles multipart upload + parse, list, metadata, parsed-content, and delete. `WorkflowSpec.DataSource` gains `kind="upload"` + `upload_id` (schema `1.2`→`1.3`); a `read_upload` agent tool is advertised only when a run declares an upload source, confined to the declared upload ids. Hardening: Content-Length checked before buffering the body, rows capped at 50,000 during parse (XLSX zip-bomb / packed-CSV guard), and the multipart `api` extra gains `python-multipart`.
- **Eval-case generation grounded in connector data.** Declared upload / MCP data sources now feed eval-case generation, and each generated case records which source it draws on. `nl_gen/input_pool.py` renders a workflow's non-standard data sources into a compact real-data context block — spreadsheets surface columns + sample rows, documents surface title + section headings, MCP sources name the connected server (`None` when the workflow has no connectors). `GeneratedEvalCase` gains an optional `input_source`, carried onto `EvalCase.input` so the replay harness knows which data to provide; the generate-eval-cases route cross-checks each case's `input_source` against the workflow's declared data-source ids before persist and clears phantom ids the model invents (LLM trust boundary).
- **Generic event-trigger framework — webhook, cron, and threshold patterns.** New `apps/kernel/src/ownevo_kernel/triggers/` (models, registry, dispatcher, webhook, cron, threshold, actions, scheduler). Migration `0032_triggers.sql` adds `trigger_definitions`, an append-only `trigger_fires` log, and a `metric_samples` table. REST at `/api/workflows/{id}/triggers` (CRUD + manual fire + fire history), an inbound HMAC receiver at `/api/triggers/webhook/{id}` (constant-time compare + replay window), and `/api/workflows/{id}/metric-samples` for threshold evaluation. A `TriggerScheduler` background task drives cron ticks, threshold polling, and Slack / email / calendar polls — opt-in via `OWNEVO_TRIGGER_SCHEDULER=true` so `make api` stays quiet by default. `WorkflowSpec` gains an additive `triggers: list[TriggerSpec]` field (schema `1.3`→`1.4`). New optional extras: `triggers` (croniter) and `otel-emit` (opentelemetry-sdk + OTLP HTTP exporter).
- **Slack channel ingestion → production-failure clustering.** `SlackIngester` polls a Slack channel through the MCP binding, converts matching messages to `production_failure` `AgentEvent`s, and feeds them into the existing clustering pipeline. Cursor-based deduplication, an optional regex filter, and a `lookback_hours` fallback on first poll.
- **Email thread ingestion (Gmail + Outlook).** `EmailIngester` subscribes to a Gmail label or an Outlook folder via MCP and turns matching messages into failure events. Per-trigger seen-set deduplication and a subject-pattern filter.
...
### Added
- **Agent registry — every connected agent as a first-class entity.** A new `agents` table (migration `0028_agent_registry.sql`) gives each workflow a stable registry row that persists across config edits, re-imports, and rebuilds. `origin` spans the full vocabulary (`greenfield` / `langsmith` / `copilot_studio`); `identity_hash` is minted once at registration and never changes. `status` supports three lifecycle states (`active` / `paused` / `archived`) with a `status_updated_at` timestamp. Three live-computed metrics are derived on every read from `iterations` and `eval_cases` — `iteration_count`, `eval_coverage_count`, `last_iteration_at` — so the registry never carries counters that can drift. New `ownevo_kernel.agents` module (`registry.py`, `models.py`) exposes `register_agent`, `register_agent_for_workflow` (idempotent; reads the workflow row and derives name + origin so callers only need a `workflow_id`), `set_agent_status`, `list_agents` (optional `origin=` / `status=` filters, AND-ed), `get_agent`, and `get_agent_by_workflow`. Three HTTP endpoints under `GET|PATCH /api/agents`: list all agents with query filters, fetch one agent by id (404 if gone), and transition the lifecycle status (PATCH returns 503 under `DEMO_MODE`; 422 for unrecognised status values). Registration is wired into every creation path: the NL-gen generate route, the design-agent import route, the OTLP ingest route (with a corrected call order: `_apply_provenance_hints` now runs before `register_agent_for_workflow` so the origin is already written when the agent row is minted), and the seven-day replay seeder. An idempotent backfill `INSERT … ON CONFLICT DO NOTHING` in the migration registers every pre-existing workflow on deploy so the Agents page is never blank. Web: a new `GET /workspaces/[ws]/agents` page renders a sortable agent table (name, origin, status, iterations, eval coverage, last iteration) with an inline status control; an Agents link is added to the workspace nav.
...
- **OTLP-HTTP protobuf ingest.** The receiver now accepts `Content-Type: application/x-protobuf` in addition to JSON — the default encoding for OpenLLMetry / traceloop and most stock OTel SDKs (langsmith-collector-proxy emits JSON). New `protobuf_decode.py` parses an `ExportTraceServiceRequest` and converts it to the same object model the JSON path feeds the mapper (rewriting base64 trace/span ids to the hex the mapper expects), then reuses the mapper unchanged. `opentelemetry-proto` added to the `api` extra; parity is proven by round-tripping every JSON fixture through protobuf and asserting identical `AgentEvent`s.
- **Production trace → failure clustering.** New `clustering/from_traces.py` reads a workflow's production traces (`iteration_id IS NULL`) whose events carry a `tool_call_result status='error'`, builds one failure snapshot per failing trace (signature from error class + tool name + truncated message; sandbox-runtime errors scored more severe than logical ones), runs the standard embed → reduce → cluster → label pipeline, and persists surviving clusters with per-cluster `sample_trace_ids`. Exposed on demand via `POST /api/workflows/{id}/cluster-production-failures` (blocked under `DEMO_MODE`) — not auto-on-ingest, since clustering is heavy and batching produces better clusters. This makes OTLP-ingested agent failures clusterable end-to-end; previously they landed in `traces` but had no path into `failure_clusters`.
- **OpenLLMetry → receiver → clustering integration test.** A real traceloop-instrumented agent (offline; mocked LLM transport) emits OTLP-protobuf to the receiver, the decoded events land in the bound workflow's traces, and the production-failure extractor clusters the broken-tool runs. Surfaced and fixed a real-output mismatch: OpenLLMetry sends `gen_ai.output.messages` as a JSON-encoded string, which the mapper now decodes (it previously handled only the nested OTLP `arrayValue`/`kvlistValue` form the hand-crafted fixtures used). `traceloop-sdk` added as the opt-in `otel-integration` extra; the test skips when it isn't installed.
- **LangSmith fix delivery — approved fixes ship back as new prompt versions.** New `POST /api/proposals/{id}/ship-langsmith` pushes a deployed skill fix to the customer's LangSmith workspace as a new Prompt Hub commit, via a thin adapter over the langsmith SDK's `push_prompt` (`middleware/langsmith_push/`, pinned in `MAPPING_PUSH.md`). Preconditions are enforced (deployed skill proposal, `workflows.origin='langsmith'`, the skill bound to a `langsmith_prompt_id`, a configured credential) with clear 4xx codes; the push is idempotent (a prior `fix-shipped-langsmith` audit entry short-circuits, and a unique index in migration `0024` enforces ship-once at the DB level) and never rolls back the deploy on failure. Success writes a hash-chained `fix-shipped-langsmith` audit entry (new audit kind, migration `0023`) carrying the LangSmith commit hash + URL. Migration `0022` adds `workflows.origin`, `skills.langsmith_prompt_id`, and an `integration_credentials` table; the LangSmith API key is encrypted at rest with Fernet under `OWNEVO_CREDENTIALS_MASTER_KEY` (`secrets/encrypted_field.py`, `make gen-credentials-key`) and never returned by the API. `workflows.origin` is auto-tagged and `skills.langsmith_prompt_id` auto-bound from ingested span attributes (best-effort; manual override wins). Web: Settings → Integrations → LangSmith (set / test / remove key), a "Ship fix to LangSmith" action on deployed skill proposals, and a per-skill LangSmith binding card on workflow Settings. A weekly `langsmith-push-conformance` workflow validates the real API but stays dormant until `OWNEVO_LANGSMITH_TEST_API_KEY` is set. Customer walkthrough in `docs/runbooks/ship-fix-to-langsmith.md`.
- **Microsoft Copilot Studio integration — Entra auth, eval-case push, definition export, and plain-language fix delivery.** New `apps/kernel/src/ownevo_kernel/middleware/copilot_studio/` (pinned in `MAPPING.md`) reaches three Power Platform surfaces over raw `httpx` behind one Entra ID seam: `auth.py` mints a service-principal bearer token via the OAuth2 client-credentials grant (scoped `{environment_url}/.default`, cached with expiry skew); `evaluation_api.py` pushes a failure cluster's eval cases into the customer's deployed agent as a Power Platform Evaluation API test set (`POST .../testsets?api-version=2024-10-01`) and backs the Settings "test connection" action with a token-only round-trip; `solutions_alm.py` exports the agent definition via the Dataverse `ExportSolution` action and best-effort-extracts the instruction text from the solution zip (a miss falls back to the trace-only summary — definition export is never a hard dependency). The structured credential (tenant / client id / secret / environment URL) is sealed as a single encrypted JSON blob into `integration_credentials` (reusing the Fernet-at-rest path) and never returned by the API. Approved fixes are delivered through `POST /api/proposals/{id}/ship-copilot-studio` — Microsoft exposes no fix-feedback API, so this makes **no external call**: it records the plain-language summary + new instruction text as a hash-chained `fix-exported-copilot-studio` audit entry (new audit kind, migration `0026`; partial unique index `0027` enforces deliver-once) for the customer to apply by hand in Copilot Studio. Preconditions (deployed skill proposal, `workflows.origin='copilot_studio'`) return clear 4xx; delivery is idempotent. Web: Settings → Integrations → Copilot Studio (set / test / remove the service principal) and a "Deliver fix to Copilot Studio" action on deployed skill proposals that renders the diff to apply. The trace-import connect flow is wired to the adapter: a new `POST /api/integrations/copilot-studio/export-definition` exports a named solution and returns the extracted instruction text, and the `/workflows/connect` Copilot Studio source (with an optional solution-name field) grounds the design agent's reverse-discovery in that definition instead of `null`; `POST /api/design-agent/import-generate` now accepts an `origin` so a Copilot Studio-imported workflow is tagged `origin='copilot_studio'` at creation (previously NULL), which is what the "Deliver fix" precondition matches against. Unit tests mock the `httpx` transport (no tenant required); a live conformance path is deferred behind a developer tenant + registered service principal, the same way the LangSmith conformance workflow is. Not yet wired (no OTel egress on Microsoft's side): Copilot Studio trace *ingest*, so the full end-to-end loop awaits a tenant.
- **Copilot Studio eval-case push call site.** New `POST /api/workflows/{id}/push-eval-cases-copilot-studio` turns a workflow's (or one failure cluster's) eval cases into a Power Platform Evaluation API test set against the customer's deployed agent, wiring up the previously-unused `create_test_set` adapter primitive. The route resolves the stored Copilot Studio credential, maps each ownEvo eval case (`input` / `expected_behavior`) onto the Evaluation API's `{input, expected_output}` shape, and pushes them under a named test set; the `agent_id` is supplied per request (Copilot Studio has no trace egress to auto-tag it yet), and an optional `cluster_id` / `test_fold_only` narrows the set. Preconditions return clear 4xx (404 unknown workflow / 422 non-Copilot-Studio origin / 422 no matching cases / 404 no credential); adapter failures map to their status (401 / 404 / 429 / 502). Success writes a hash-chained `eval-cases-pushed-copilot-studio` audit entry (new audit kind, migration `0028`) recording the created test-set id + case count, rendered on the activity feed and audit trail. The test-run / result-poll lifecycle is deliberately out of scope — those Evaluation API shapes are preview and unpinned. Unit tests mock the adapter (no tenant required); a live conformance path awaits a developer tenant, like the LangSmith and Copilot Studio fix-delivery surfaces.
- **IBM watsonx Orchestrate ADK ingest adapter (translator slice).** New `apps/kernel/src/ownevo_kernel/middleware/watsonx_adk/` rewrites Traceloop / OpenLLMetry vendor attribute keys onto the standard GenAI Semantic Conventions so OpenLLMetry-instrumented agents (watsonx Orchestrate ADK uses Traceloop under the hood) decode into the same `AgentEvent` stream as a Claude-SDK or LangChain trace. Bridges five divergences: `traceloop.span.kind = "tool"` → `gen_ai.operation.name = "execute_tool"`; `traceloop.entity.name` → `gen_ai.tool.name`; `traceloop.entity.input` → `gen_ai.tool.call.arguments`; `traceloop.entity.output` → `gen_ai.tool.call.result`; and a synthetic deterministic `gen_ai.tool.call.id` derived from the span id (OpenLLMetry does not emit a tool-call id since the Traceloop SDK identifies tool invocations by span position rather than an explicit correlation id). Orchestration spans (`traceloop.span.kind = "workflow" | "agent" | "task"`) map onto `invoke_workflow` / `invoke_agent` so the receiver's silent-drop branch handles them as trace anchors without producing AgentEvents. The rewrites are conditional (a future OpenLLMetry release that ships native semconv keys becomes a no-op) and non-destructive (deep-copies its input). Wired into `/api/otel/v1/traces` alongside the Google ADK translator — order is irrelevant since each targets a disjoint vendor namespace. The receiver's `MAPPING.md` gains a watsonx entry under "Vendor-specific adapters". Tests: 15 translator units covering each rewrite, conditional-overwrite invariants, snake_case envelopes, non-Traceloop pass-through, and the end-to-end chain `watsonx payload → translate → decode_otlp_payload` producing `ToolCallStart` + `ToolCallResult` with the correct args/output. Live integration test against a real watsonx Orchestrate environment is deferred until a design-partner deployment is available; the translator is shape-only and exercised against hand-crafted OTLP-JSON fixtures matching the Traceloop SDK's documented emission shape.
- **Google Agent Development Kit ingest adapter.** New `apps/kernel/src/ownevo_kernel/middleware/google_adk/` rewrites the two ADK vendor-prefixed attribute keys (`gcp.vertex.agent.tool_call_args` / `gcp.vertex.agent.tool_response`) onto the standard GenAI Semantic Conventions equivalents (`gen_ai.tool.call.arguments` / `gen_ai.tool.call.result`) so the existing OTel receiver decodes ADK-emitted spans into the same `AgentEvent` stream a LangChain or OpenLLMetry agent produces. The translator is conditional (a future ADK release that ships standard keys is a no-op) and non-destructive (deep-copies its input). Companion `readable_spans_to_otlp_json` encodes `opentelemetry.sdk.trace.ReadableSpan` objects (as captured by `InMemorySpanExporter` in tests) into the OTLP-JSON envelope the receiver mapper consumes, with nested dicts encoded as `kvlistValue` so attributes like `gen_ai.output.messages` round-trip into a Python dict rather than a JSON string. The receiver's `MAPPING.md` gains a "Vendor-specific adapters" section pointing at this module. New optional `[adk]` extra pins `google-adk>=2.1,<3` + `opentelemetry-sdk>=1.36,<2`. Tests: 13 translator/encoder units covering vendor-key rewrites, no-mutation invariants, partial vendor keys, and the end-to-end chain `ADK payload → translate → decode_otlp_payload` producing `ToolCallStart` + `ToolCallResult` with the correct args/output; one live integration test (gated on `GOOGLE_API_KEY` and `google-adk` being importable) that drives a real Gemini call through `InMemoryRunner`, captures the emitted OTel spans, and asserts the round-tripped `AgentEvent` stream contains the agent's tool call.
- **`sim_tier='mock'` — deterministic scripted simulation tier (both layers).** A new `workflows.sim_tier` discriminator (`'real' | 'mock' | 'replay'`, default `'real'`) selects how a workflow's iterations execute. The `mock` tier returns deterministic scripted responses with zero LLM cost and sub-second per-case latency, for fast inner-loop iteration. Two layers ship: the **solver layer** — `iteration_runner` routes `sim_tier='mock'` workflows to a mock solver that emits scripted labels from `mock_sim_config`; and the **sandbox layer** (`sandbox/mock_sim.py`, for benchmark/code-gen workflows) — a `SandboxRuntime` implementation that returns scripted stdout/stderr per tool call instead of spinning a Docker container. New `ownevo_kernel.sim_tier` module owns the `SimTier` enum + config parsing. (See the migration entry below for the schema.)
- **`sim_tier='replay'` — byte-identical iteration replay with zero LLM agent calls.** A third simulation tier that replays a prior real iteration's captured agent predictions instead of calling an LLM. New `ReplayAgentSolver` (`eval_runner/replay_solver.py`) reads per-case structured outputs from `iteration_case_outputs` and emits them as `ReplayResult` objects, prepending a `[replay]` marker on each rationale so the audit trail surfaces the capture origin. `expected_value` is taken from the live case set so replays reflect today's gate semantics even when the eval suite has evolved. Cross-workflow ownership is enforced at the query layer (JOIN to `iterations`) so a mis-pointed `source_iteration_id` from another workflow returns empty results rather than silently serving foreign predictions. Fallback policy `ReplaySimConfig.fallback` supports `'error'` (Slice A, raises `ReplayCaseMissingError` listing the uncovered case_ids), `'mock'`, and `'real'` (Slice B, documented but guarded by an explicit `WorkflowNotIterableError`). New `ReplaySimSandbox` (`sandbox/replay_sim.py`) replays captured Docker outputs from `captured_sandbox_runs` in `call_idx` order — each instance starts at cursor 0 so a fresh `ReplaySimSandbox` per iteration aligns with the `CapturingSandbox` that wrote the rows. `sim_tier.py` parses `MockSimConfig` and `ReplaySimConfig` from `workflows.sim_tier` / `mock_sim_config` / `replay_sim_config`; `iteration_runner` uses `contextlib.nullcontext()` for the no-capture code path instead of a hand-rolled async context manager.
- **`CapturingSandbox` — decorator that records every sandbox `.run()` to `captured_sandbox_runs`.** Wraps any `SandboxRuntime` (LocalDockerSandbox in practice) and persists each result to the `captured_sandbox_runs` table before returning. Uses a `field(default=0, init=False)` monotonic `_call_idx` counter (prevents accidental construction-time override; aligns with `ReplaySimSandbox`'s cursor). Capture write failures are non-blocking — the inner result is always returned; errors are logged and counted on `capture_failures` so smoke tests can assert clean runs. Plain INSERT (no ON CONFLICT) treats a `(iteration_id, call_idx)` duplicate as a programming error (loud `IntegrityError`) rather than silently overwriting prior captures.
- **Migration `0020_mock_sim_tier.sql` + `0021_replay_sim_tier.sql`.** `0020` adds `workflows.sim_tier` enum `('real','mock','replay')` with default `'real'`, `mock_sim_config jsonb`, and the `MockSimSandbox` table with HSTORE-style label maps. `0021` adds `workflows.replay_sim_config jsonb` (with a CHECK that `sim_tier<>'replay' OR replay_sim_config IS NOT NULL`) and the `captured_sandbox_runs` table with `UNIQUE(iteration_id, call_idx)`. (Migrations renumbered from `0018`/`0019` to avoid collision with the OTel `0018_traces_ingest_source.sql` migration that landed on main simultaneously.)
- **End-to-end replay smoketest** (`scripts/sim_replay_smoketest.py`, `make sim-replay-smoketest`). Seeds a fresh isolated Postgres database with a workflow, iteration, and N captured `iteration_case_outputs` rows, drives `run_with_replay_agent` against them, and asserts every prediction replays byte-identically with the `[replay]` rationale marker. Fixture-owned cases (not in the captured set) must appear in the returned `missing` list. Budget gate: wall-clock ≤ 5 s (no LLM calls). The test DB is always dropped in the `finally` block; a failed `asyncpg.connect` also cleans up before returning non-zero.
- **OTLP-JSON ingest receiver — external collectors can now feed ownEvo.** New `POST /api/otel/v1/traces` accepts a single OTLP `ResourceSpans` batch (the encoding `langsmith-collector-proxy` and OpenLLMetry emit by default when targeting a non-gRPC sink). Body is streamed with an 8 MiB cap (`_read_body_with_limit` aborts mid-chunk on overage; Content-Length fast-path short-circuits well-behaved callers), CPU-bound JSON decode is offloaded via `asyncio.to_thread`, and Pydantic `ValidationError` is caught per-span so bad external payloads produce DecodeWarnings instead of 500s. New `apps/kernel/src/ownevo_kernel/middleware/otel_receiver/` package: `mapper.py` (OTel GenAI span → `AgentEvent`; covers `chat` / `text_completion` / `generate_content` → ContentDelta + optional ReasoningDelta; `execute_tool` → ToolCallStart + ToolCallResult; `retrieval` → Citation events; `invoke_agent` / `invoke_workflow` / `create_agent` / `embeddings` → silent pass-through), `receiver.py` (sync + async entry points), `MAPPING.md` (canonical OpenTelemetry GenAI Semantic Conventions v1.27 ↔ `AgentEvent` table). STATUS_CODE_UNSET separated from OK so tool spans carrying numeric `0` are flagged with a warning (protects failure-clustering data integrity); `ownevo.error_class` over external OTLP emits a provenance warning since external callers should not silently reclassify sandbox errors; retrieval document lists capped at `_MAX_RETRIEVAL_DOCS_PER_SPAN` (1 000) to bound memory amplification; UUID hex decoding is field-context-aware (8 bytes for span_id, 16 for trace_id) to prevent namespace collisions.
- **OTLP ingest persistence — ingested traces are first-class.** Decoded `AgentEvent` streams now write through to the same `traces` row that the in-process `TraceCollector` produces. One row per unique `AgentEvent.trace_id` in the batch, tagged `ingest_source='otlp'` (vs `NULL` for kernel-emitted). Subsequent batches that share a `trace_id` append onto the existing row's JSONB events array via `INSERT ... ON CONFLICT (id) DO UPDATE SET events = traces.events || EXCLUDED.events` — matches the wave-flush pattern of real OTel collectors where spans for one trace arrive across multiple batches as each completes. New `middleware/otel_receiver/persist.py` runs the whole batch inside a single `conn.transaction()` so a mid-batch failure rolls back every prior upsert; a per-trace event-count cap (`_MAX_EVENTS_PER_TRACE = 10 000`) guards against unbounded JSONB growth from buggy or malicious collectors re-POSTing the same trace_id, with saturated batches surfaced as `saturated_trace_ids` in the response. Migration `0018_traces_ingest_source.sql` adds the column with a `CHECK ... NOT VALID` constraint then explicit `VALIDATE` so the existing-rows scan runs under SHARE UPDATE EXCLUSIVE — safe online on the high-volume traces table. The route response surfaces `created_trace_ids` + `appended_trace_ids` + `saturated_trace_ids` so a calling collector can route "new trace" notifications or pause emission against saturated traces without re-querying the database. Ingested traces are visible through `GET /api/traces` and `GET /api/traces/{id}` immediately — the failure clustering pipeline, the gate runner, and the trace inspection UI all read them through the existing read path.
- **Auto-clustering of ingested production failures (debounced).** New `apps/kernel/src/ownevo_kernel/clustering/auto_trigger.py` closes the loop between OTLP ingest and the failure-clustering pipeline: the ingest route signals a bound workflow as needing (re)clustering whenever a batch lands a `tool_call_result status='error'`, and a single in-process background task — started in the app lifespan — runs `cluster_production_failures` for a workflow once it has been quiet for a debounce window (default 30 s). Debouncing is deliberate: the pipeline is CPU-heavy (sentence-transformers + UMAP + HDBSCAN + an LLM label call) and external collectors flush one trace's spans in waves, so coalescing a burst into one run is both cheaper and produces better clusters than re-clustering per flush. Runs are sequential (no two heavy pipelines load embedder models concurrently); a signal arriving mid-run re-marks the workflow so traces landing during a run are not missed; a failed run is logged and skipped without crashing the loop; and a missing `clustering`/`agent` extra disables the trigger after the first attempt rather than logging on every ingest. Opt-in via `OWNEVO_CLUSTER_AUTOTRIGGER=true` (off by default so `make api` doesn't load embedding models or incur LLM-label cost unprompted; the on-demand `POST /api/workflows/{id}/cluster-production-failures` endpoint is unchanged), tunable through `OWNEVO_CLUSTER_AUTOTRIGGER_DEBOUNCE_SECONDS`, and never started under `DEMO_MODE`. State is in-process — a restart drops pending signals but self-heals on the next ingest, and nothing is lost permanently since clustering re-derives from the `traces` table. Tests: 8 coalescer units (burst coalescing, mid-run re-queue, multi-workflow, ImportError-disables, exception-swallowing, start/stop lifecycle) plus 3 route-wiring tests (failing batch marks the workflow pending, a clean batch does not, disabled-by-default starts no trigger).
- **Trace-import discovery prompts for the design agent.** New `get_trace_import_discovery_questions()` in `apps/kernel/src/ownevo_kernel/design_agent/prompts/` returns the question set the design agent uses when a customer brings an existing agent's trace export rather than designing a new agent from scratch. The discipline (metric negotiation, ambiguity surfacing) carries over from the authoring-time discovery flow; the prompts differ because the starting material is observed agent behaviour, not a target description. Web-side UX is a follow-up.
- **Trace-import design surface — connect an existing agent, then define it from its own traces.** New `/workspaces/[ws]/workflows/connect/design` runs the design-agent discovery interview over an agent's ingested traces instead of a written description, and reverse-engineers a WorkflowSpec from them on Generate. Kernel side: `design_agent/trace_summary.py` builds a deterministic, prompt-ready summary of the imported traces (tool-usage rollup, sample arguments/outputs, observed failure modes) from the stored `AgentEvent`s; `design_agent/trace_interviewer.py` (`pick_next_question_from_traces`) is an LLM interviewer that grounds each decision brief in that observed behaviour plus an optional exported agent definition, reusing the authoring interviewer's tool schema, `QuestionBrief` shape, and seven-dimension coverage logic; `nl_gen/workflow_spec_from_traces.py` reverse-engineers a typed `WorkflowSpec` from the trace summary using the same `emit_workflow_spec` tool + validation-retry machinery as the authoring generator, after which the existing (spec-coupled) simulation-plan and metric generators and the workflow-persistence + `design-agent-negotiation` audit path are reused unchanged. Two endpoints under the existing `design_agent` surface: `POST /api/design-agent/import-next-question` (`{trace_ids[], agent_definition?, prior_answers[]}` → next decision brief; falls back to the static trace-import prompt set when no LLM is available so the interview never blocks) and `POST /api/design-agent/import-generate` (`{trace_ids[], agent_definition?, design_agent_log, workflow_id?}` → loads + summarises the traces, generates spec/sim/metric, persists the workflow, and mirrors the discovery transcript into the audit chain). Web: a three-pane flow (imported-trace summary + agent definition · discovery chat · progress/transcript + Generate) reusing the authoring decision-brief components, and the Connect source picker now routes the OTel source to this surface (ingestion is wired) instead of a placeholder. Tests: unit coverage for the summary builder, the trace interviewer (mocked LLM), the spec-from-traces generator (mocked LLM against a known-valid spec), and the `import-next-question` endpoint (mocked pool, static-fallback walk). Exported-definition reverse-discovery and the import-specific audit kind are follow-ups.
- **Reverse-discovery opening turn for the trace-import design surface.** The trace-import conversation now opens by telling the reviewer what the imported agent *appears to do today* — a one-to-two-sentence "this agent does X" inference — and asks them to confirm or correct it before the dimension interview begins. The confirmed (or corrected) text becomes the agent definition that grounds the rest of discovery and the generated spec. Kernel: `design_agent/reverse_discovery.py` produces the summary via a single plain-text Anthropic completion grounded in the deterministic `TraceSummary` (observed tools, outputs, failure modes) plus an optional exported agent definition; `fallback_reverse_discovery_summary()` renders a deterministic "calls `a`, `b` and N other tools across M traces" summary when no LLM is available or the call fails, so the conversation is never blocked. New endpoint `POST /api/design-agent/import-summary` (`{trace_ids[], agent_definition?}` → `{summary, basis, source}` where `basis` is `traces`/`definition+traces` and `source` is `llm`/`fallback`). Web: `/workspaces/[ws]/workflows/connect/design` opens on a confirm-or-correct card; the first discovery question is fetched after confirmation so it is grounded in the confirmed definition. Tests: unit coverage for the summary generator and deterministic fallback, plus the `import-summary` endpoint (fallback path, definition basis, unknown-trace 404). Persisting the summary to a dedicated import audit log is a follow-up.
- **Import discovery audit log — the trace-import conversation is now persisted to its own auditable record.** Trace-import authoring previously reused the written-description path's `design-agent-negotiation` audit rows, and the reverse-discovery turn (the inferred "this agent does X" plus whether the reviewer confirmed, corrected, or skipped it) was not persisted anywhere. Both gaps are closed. Migration `0025_design_agent_import_log.sql` adds a `workflows.design_agent_import_log` JSONB column and a new `audit_kind` value `design-agent-negotiation-import`. New module `design_agent/import_log.py` (`DesignAgentImportLog`, `ReverseDiscoveryRecord`, `persist_design_agent_import_log`): at WorkflowSpec finalization it writes the reverse-discovery turn + full Q/A transcript to the JSONB column and mirrors them into the hash-chained audit trail — one `design-agent-negotiation-import` row for the reverse-discovery turn (carrying the inferred summary, basis, source, and the reviewer's `confirmed`/`corrected`/`skipped` decision) and one per discovery Q/A — so import-originated negotiation stays queryable apart from the written-description path. `POST /api/design-agent/import-generate` accepts an optional `reverse_discovery` field and persists the import log in the same transaction as the workflow-row INSERT (skipped entirely when there is nothing to record). Web: the connect/design flow records the reviewer's confirm/correct/skip decision and echoes it back on Generate. Tests: model-level coverage (validation, `is_empty`, JSONB round-trip), DB-backed persistence coverage (column + audit rows, kind isolation, actor, missing-workflow), and API-level coverage that the route builds and persists the import log (and skips when empty).
- **Failure provenance + flat-list view on the Failures tab.** Every failure now carries a `production` / `eval` tag derived from `traces.iteration_id IS NULL`, with no schema change. The cluster endpoint returns `prod_count` + `eval_count` per cluster (computed by joining each cluster's `sample_trace_ids` back to `traces`). A new endpoint `GET /api/workflows/{id}/failures` returns one row per sample trace across all clusters with a `?source=production|eval` filter. UI: each cluster card grows source pill(s) — two pills with counts on mixed clusters ("Prod 3" + "Eval 2"), one unmarked pill on single-source clusters, nothing on legacy clusters with no underlying trace rows. New control strip with an All / Production / Eval filter and a Cluster view / List view toggle (URL-search-param-driven, shareable links). Flat-list view renders timestamp / source / severity / cluster / iteration / eval-case columns ordered newest-first. The demo seeder also inserts mixed-source trace rows alongside the seeded clusters so the demand-prediction demo workspace shows realistic pills out of the box.
- **Workflow Spec tab — every generated artifact visible 30+ days post-creation.** New route `/workspaces/[ws]/workflows/[wf]/spec` reuses the four `gen-section` blocks from the authoring-time review page (Simulator / Eval cases / Success metric / Operate-view UI) — same provenance lines, same design-agent attribution callouts. Section renderers extracted to `apps/web/app/components/workflow-spec-sections.tsx` so the authoring and post-creation surfaces share a single source of truth. Added between Overview and Operate in the workflow tab strip.
- **Inline-editable description on Overview and Spec.** Two distinct paths: **Quick edit** (existing direct PATCH, cosmetic) and **Propose change** (new gate-routed flow). The inline block toggles between read mode (description body + two corner buttons) and edit mode (textarea + save / cancel) without leaving the page; substantive rewrites flow through the proposal queue.
- **`proposal_kind` discriminator for non-skill artifact edits.** Migration `0017_proposal_kind.sql` adds an enum `('skill', 'description', 'metric', 'sim', 'ui-primitive')` with default `'skill'`, a `proposed_payload jsonb` column for artifact-shaped non-skill content, and CHECK constraints pinning that `kind='skill'` keeps a non-null `skill_id` while non-skill kinds carry a non-null payload. Backward compatible — existing rows default cleanly.
- **Four non-skill proposal-create endpoints.** `POST /api/workflows/{wfId}/proposals/{description,metric,sim,ui-primitive}` each accept a plain-language summary + an artifact-shaped payload + optional rationale, persist the payload in `proposed_payload`, anchor to the workflow's latest iteration (422 if no iterations), and write a `proposal-created` audit entry. Each endpoint enforces a kind-specific invariant: metric requires a non-empty `name`; ui-primitive requires `type` on every entry; description rejects no-op proposals that match the current text; sim requires at least one of `tools / personas / data_sources / env_generators`.
- **Per-artifact diff renderers on the proposal-detail page.** Replaces the generic JSON-dump fallback for non-skill kinds. **Description** renders a side-by-side Current vs Proposed view with a character-delta header (the proposal payload carries both the new and previous text so no second fetch is needed). **Metric** renders a labelled definition card (name + family + direction + description + rationale). **Sim** computes added / removed entities per section (tools / personas / data_sources / env_generators) by identifier and renders pill rows; sections present in the proposal with no delta show a "may update props on existing entries" hint. **UI primitive** computes added / removed / unchanged types and renders three pill rows. The proposal header swaps the "Skill" meta line for an "Artifact" line on non-skill kinds.
- **Ordering-inversion check for metric proposals.** New endpoint `GET /api/proposals/{id}/ordering-inversion-check` (only for `kind='metric'`) re-scores every prior iteration on the workflow under both the current and the proposed metric using `nl_gen.metric_compute.compute_metric` — the same function the regression gate calls, so the check shares metric semantics with the gate. The proposed-metric payload from the Spec-tab form (name + family + direction + description) is inflated into a full `MetricDefinition` by inheriting required fields (`workflow_spec_id`, bounds, `target_value`) from the workflow's current metric so the check still runs against incomplete user-supplied payloads. UI: new `OrderingInversionPanel` above the approval form renders per-iteration deltas + verdict pills; orange-tinted heading when any iteration would flip pass/fail under the new metric, green when none. Returns `status='unavailable'` with a reason string when the check can't run (no current metric definition, no iterations, no bool labels in case outputs) so the panel renders gracefully on every path.
- **A/B framing — non-skill proposals stage on approve, apply on deploy.** Approve now transitions non-skill proposals to `approved-awaiting-deploy` (same as skill kinds); the workflow row is unchanged until an explicit Deploy step fires. Deploy dispatches through `APPLY_BY_KIND` (`apps/kernel/src/ownevo_kernel/approvals/apply.py`) — `description` writes `workflows.description`; `metric` merges the form-shaped payload onto the current `metric_definition` (preserving bounds, target_value, schema_version), mapping the form's `higher` / `lower` direction labels to the schema's `maximize` / `minimize`; `ui-primitive` replaces `spec.ui.tabs[0].primitives`; `sim` merges whichever of `tools / personas / data_sources / env_generators` the payload carries into `spec` (untouched sections survive). The UPDATE runs in the same transaction as the state transition — an apply failure rolls back the whole decision atomically. The "production interval" is the time between approve and deploy: a reviewer can validate the proposed change against the current value (the ordering-inversion check stays available for metric proposals through this window) before flipping the default.
- **`DeployForm` kind-aware help text.** "Sets the skill's deployed_version_id..." copy stays on skill kinds; non-skill kinds read "Applies the proposed {kind} change to the workflow row. The change is live as soon as Deploy fires." Rollback button hides for non-skill kinds — the documented path is to create a fresh proposal pointing at the previous value.

### Changed
- **Shorter demo invite-token payload.** `mint_invite_token` now emits single-letter claim keys (`l` / `t` / `e` / `j` / `i` for label / tier / exp / jti / iat), shrinking the encoded invite URL by 15–20 chars without changing the signing format or wire protocol (still a base64url payload + base64url signature joined by a dot). `verify_invite_token` canonicalizes short keys back to long-form before validation, so tokens minted before this change keep validating until their TTL runs out — no re-mint or rotation required.
- **`Proposal.skill_id` is now optional** at the type and API layers. Required for `kind='skill'` (enforced by a DB CHECK constraint); `None` for the four non-skill artifact kinds. `DeployResponse.skill_id` and `ProposalDetail.skill_id` follow the same shape.

### Fixed
- **`-- ownevo:no-txn` migrations were never actually applied correctly.** The migration runners (`scripts/migrate.py` and the test-bootstrap `db.migrate`) passed the whole annotated file to a single `execute()`, which the driver sends as one multi-statement string — Postgres wraps that in an implicit transaction, so `CREATE INDEX CONCURRENTLY` errored and `VALIDATE CONSTRAINT` silently lost its lighter lock. Because the only no-txn migration so far (`0018b`) errored at fixture setup, the entire DB-backed test suite had been erroring rather than running whenever `OWNEVO_DATABASE_URL` was set. Both runners now split such files into individual statements (`split_sql_statements`) so each runs in autocommit.
- **`EntryStrip` Greenfield card on `/workflows/connect`** was a non-interactive `<div>` with the `dim` class, leaving the on-ramp's back-link to the description path unreachable. Both EntryStrip cards are now `<Link>` elements with consistent active-vs-inactive styling. Also stopped mislabelling "← you are here" as Connect on the new-workflow page when arrived at via `?from=connect` — the user is on Greenfield, and the Connect on-ramp context note already renders separately.

## [0.11.0] — 2026-05-23

### Added
- **README opener with four product screenshots** (`docs/img/workflow-overview.png`, `design-agent.png`, `proposal-review.png`, `audit-trail.png`), captured from a local `make seed-demo-with-iter` instance with the 4-iteration climbing fixture (val_score 0.46 → 0.65 → 0.77 → 0.80 on `demand-prediction`). New "What the domain expert actually sees" section between the system tour and Quick start. `Live demo` link added to the top nav row pointing at `demo.ownevo.ai`. GitHub repo description + homepage + topics set out-of-band: topics now include `agent-evaluation`, `ai-agents`, `audit-log`, `enterprise-ai`, `improvement-loop`, `llm-evals`, `regression-testing`, `supply-chain`.
- **Token-quota gate on the live-demo design + NL-gen surface.** New kernel modules `apps/kernel/src/ownevo_kernel/api/_demo_identity.py` (HMAC-signed invite tokens, anonymous cookie resolver), `_demo_quota.py` (per-`(identity_key, day)` token counter), `_demo_budget.py` (global soft gate flipped by the operator), `_demo_gate.py` (composed FastAPI dependency), and `_demo_token_accountant.py` (per-request `messages.create` interceptor). Migration `0016_demo_phase1.sql` adds `demo_usage` / `demo_invite_revocations` / `demo_budget_state`. `POST /api/nl-gen/generate` and `POST /api/design-agent/next-question` now go through `DemoGateDep`: under `DEMO_MODE=true` they refuse anonymous visitors who have spent their daily budget (429 with `code=demo_quota_exhausted`), refuse everyone when the global cap is flipped (502 with `code=demo_budget_cap_reached`), and record actual `input_tokens + output_tokens` after every LLM call via a request-scoped monkey-patch on `client.messages.create`. Outside demo mode the gate is a no-op and never touches the DB pool.
- **Per-recipient signed-invite redeem flow.** New `POST /api/demo/redeem-invite` route validates an HMAC-signed token against `OWNEVO_DEMO_SIGNING_KEY` + the `demo_invite_revocations` denylist, then sets the `ownevo_demo_invite` cookie HttpOnly. New Next.js `apps/web/middleware.ts` catches `?invite=<token>` on any URL and sets the cookie via redirect — no email service, no OAuth, no per-user persistence. Two tiers above anonymous: `elevated` (10× the anon cap) and `unlimited` (cap skipped entirely). Per-recipient revocation via `make demo-revoke JTI=...`. Signing-key rotation invalidates every extant invite simultaneously.
- **`GET /api/demo/status` + server-rendered disabled-state UX.** The new endpoint returns `{tier, used_tokens, limit_tokens, exhausted, budget_exhausted, reset_at}` for the current visitor. `apps/web/lib/demo-status.ts` reads it server-side with cookie forwarding so quota-gated CTAs (`Design with agent`, `Generate workflow`) render with `disabled=true` and an explanatory tooltip when the visitor is out of quota — visitors never click into a guaranteed-failed call and wait on a spinner. The existing `DemoBanner` is now an async server component reflecting live tier + used-% (or "daily LLM budget is exhausted — back tomorrow" when the global cap is flipped).
- **Operator CLI for demo invite + budget management.** `apps/kernel/scripts/mint_demo_invite.py` mints labeled JWTs; `apps/kernel/scripts/demo_admin.py` revokes by `jti` and flips today's `demo_budget_state` row. Wired via `make demo-invite LABEL=… TIER=… DAYS=…`, `make demo-revoke JTI=…`, `make demo-budget-cap`, `make demo-budget-clear`.
- **Benchmark workflows are hidden under `DEMO_MODE=true`.** `GET /api/workflows` filters out `kind='benchmark'`; direct fetches of a benchmark workflow return the same opaque 404 as a missing row. M5 and τ³ surfaces are kept entirely off the public demo.
- **Hand-crafted demand-prediction lift curve in the demo seed** (`apps/kernel/scripts/seed_demo.py`). Four iterations climbing val_score 0.500 → 0.650 → 0.770 → 0.800 (iter #0 gate-blocked, the rest gate-passed) plus two named failure clusters (`Holiday markdown false-negatives, weeks 47–51`; `Off-shelf bias on slow-mover SKUs`) and one gate-passed proposal tied to the latest iteration. The real iteration runner lands flat at val_score=0.8 on the seeded eval set — the cases don't differentiate iterations, so the Overview lift chart reads as "the loop doesn't improve". The hand-crafted fixture runs above the `--with-iterations` early-return so a stock `make seed-demo` produces a demoable lift chart + non-empty Failures / Proposals tabs without an `ANTHROPIC_API_KEY`. Idempotent: rows are inserted when missing and patched when present.
- **Audit chain export — workspace and per-workflow.** `GET /api/audit/export` returns the log as canonical JSON (sorted keys, no whitespace); an optional `?workflow_id=` parameter filters to entries anchored to a specific workflow. Web: `Export chain` button on `/workspaces/[ws]/audit` (workspace-scoped) and on the per-workflow `/workflows/[wf]/audit` tab (workflow-scoped). Both proxied server-side.
- **Per-workflow model routing is now load-bearing.** New `apps/kernel/src/ownevo_kernel/llm/router.py` dispatches a `provider:model` slug from `workflows.agent_model_id` to a live chat client (`AsyncAnthropic` for Anthropic, `AsyncOpenAI` for OpenAI / xAI / Gemini / OpenRouter / Local, `OllamaChatClient` for Ollama daemons). `iteration_runner.run_one_iteration_for_workflow` looks up the workflow's stored slug, builds the client through the router, and threads it into `run_nl_gen_demo_loop` so the agent solver swaps providers per workflow row. Boot-time `check_provider_api_keys` warns at startup when an enabled provider has no API key. The honesty subnote on the picker is gone — the dropdown now changes behaviour.
- **`Local / Custom OpenAI` provider** replaces Fireworks. The `local` slug dispatches to any OpenAI-compat server (LM Studio, vLLM, llama.cpp, on-premise gateway) via `OWNEVO_LOCAL_BASE_URL` → `OWNEVO_LLM_BASE_URL` → `http://$OWNEVO_LLM_HOST:1234/v1`. `OWNEVO_LOCAL_API_KEY` is optional for servers that enforce a key. `docker-compose.yml` updated to pass the new env vars through. OpenRouter enabled by default in `.env.example`.
- **`Request changes` decision on the proposal review surface** — middle-ground between Approve and Reject. New `proposal_state = 'changes-requested'`, new audit kind `proposal-changes-requested`, new endpoint `POST /api/proposals/{id}/request-changes` (comment required). The steering text is picked up by `iteration_runner` on the next cycle and threaded into `initial_instruction`. Migrations split into `0015_changes_requested_enums.sql` (ADD VALUE, autocommit) and `0015b_changes_requested_constraint.sql` (transactional) for Postgres < 16 compatibility.
- **Settings Export card.** Workflow Settings tab gains a primary `Export full bundle` button and five individual exports: Agent, Evals, Proposals, Failures, Audit. The bundle (`bundle-<wfId>-<ts>.json`) packages the workflow anatomy, all skills with head-version content, the full eval suite, improvement history (up to 500 proposals), production failure clusters, and the workflow-scoped audit trail — fanned out in parallel, gracefully degrading on partial failures. All route handlers are server-side proxies; the kernel URL never reaches the browser.
- **Proposal detail back-nav.** The middle breadcrumb on `/workspaces/[ws]/proposals/[id]` now links to the workflow's Proposals tab (was Failures), and the full `WorkflowTabs` strip renders on the page with Proposals forced active so a reviewer can return to any workflow surface without the browser back button. `WorkflowTabs` gains an optional `activeOverride` prop for surfaces that live outside the workflow route tree.
- **τ³-bench Q3 2026 results writeup** (`benchmarks/tau3-results-2026-Q3.md`). Documents the four conditions exercised — A (cloud baseline), B (cloud + judge), A-LOCAL (all-local baseline), B-LOCAL (all-local + judge) — plus NeoSigma comparison, top skill improvements, and honest disclosure of which configs reproduced lift. The τ³ master loop log now records the LLM-judge verdict per cycle (`judge=admit|reject`) when `OWNEVO_TAU3_LLM_JUDGE=1` is set. T7–T12 proposer sweep results documented in `TAU3_LOCAL_TESTPLAN.md`: MoE-a3b architecture is the load-bearing factor for proposer lift (`qwen3:30b-a3b` Ollama produces ~2× the lift of dense models in the same class), and proposer lift is task-agent-baseline-dependent — the same proposer that lifts a low-baseline solver can regress a high-baseline one (T12 vs T1).
- **Per-section design-agent attribution on the workflow review page** (`apps/web/app/workspaces/[wsId]/workflows/new/review/[wfId]/design-attribution.tsx`). Each generated section (Simulator / Eval cases / Success metric / Operate-view UI) now renders an accent-tinted callout naming the operator's discovery answers that shaped it. An operator who picked Recall-first sees that quote on the Metric card and the Eval-cases card; an operator who skipped discovery sees no callouts (component returns null when no log). Per-section dimension subsets mirror the kernel-side generator subsets (Simulator → goal + trigger + connectors; Eval → eval_seed_cases + success_metric; Metric → success_metric; UI → operate_ui_primitives).
- **Design-agent answers become load-bearing in NL-gen.** New `apps/kernel/src/ownevo_kernel/nl_gen/design_brief_context.py` walks a `DesignAgentLog` and renders a prompt-shaped markdown block scoped to a dimension subset. Each of the four generators (`generate_workflow_spec`, `generate_simulation_plan`, `generate_metric_definition`, `generate_eval_case_set`) gains a `design_brief_block: str | None = None` kwarg; when set, the block is appended to the user message with a prefix instructing the model to treat answers as hard constraints. `POST /api/nl-gen/generate` slices the incoming log into per-generator briefs; `POST /api/workflows/{id}/eval-cases/generate` loads the persisted log via `load_design_agent_log` and passes per-generator slices. `DesignAgentLogEntry` extended with optional `dimension` + `chosen_option` (legacy entries get back-compat-routed via `metric → success_metric`, `trigger → trigger_and_cadence`, etc.). Net: two operators with the same description but different priorities (e.g. Recall-first vs. Precision-first) now get materially different spec / sim / metric / eval — answers are constraints, not framing.
- **LLM-driven design-agent interview** replacing the hardcoded prompt-library walk. New `apps/kernel/src/ownevo_kernel/design_agent/dimensions.py` declares seven design-shaping dimensions (`goal_and_scope`, `trigger_and_cadence`, `data_sources_and_connectors`, `success_metric`, `eval_seed_cases`, `operate_ui_primitives`, `reviewer_role`), each with an intent prose block + the NL-gen artifacts it informs. New `design_agent/interviewer.py` computes coverage from `prior_answers`, forced-tool-calls Sonnet 4.6 with an `ask_question` tool, validates the response against a `QuestionBrief` shape (question + ELI + stakes + 2-4 options each with pro/con + recommendation index + rationale), returns None when every dimension is covered. `POST /api/design-agent/next-question` routes through the interviewer by default; falls back to the hardcoded prompt library only when `ANTHROPIC_API_KEY` is missing OR the LLM call fails OR validation fails. Tagged `source: "llm"` vs `source: "fallback"` on the wire.
- **Decision-brief surface on `/workflows/new/design`.** Dimension coverage strip across the top of the centre pane (seven chips flipping pending → current → done). Current question rendered as a full decision-brief card: dimension badge + source badge, large question, ELI subtitle, amber-tinted Stakes callout, full-width option cards each showing label + ✓ pro + ✗ con visible, recommended option outlined in accent with a Recommended badge, recommendation-rationale callout grounded in the description's own phrasing. Past Q&A collapses from big bubble pairs to compact one-liners.
- **Domain-shaped output payloads end-to-end on the Operate tab.** Migration `0013_case_output_payload.sql` adds `output_payload jsonb` on `iteration_case_outputs`. The `predict_label` agent tool gains a required `output_payload_json` field — a JSON-encoded string whose top-level keys match what the workflow's Operate-tab primitives expect (`metrics`, `time_series`, `table`, `alerts`, `kanban`, `schedule`, `conversation`, `side_by_side`, `document`). `AgentPrediction` / `ReplayResult` / `EvalCaseOutcome` thread the decoded payload through to the new column. The layer-D resolver in `apps/web/lib/primitive-data-resolver.ts` gains new operate-context branches: when `context='operate'`, each declared primitive reads `output_payload` across the iteration's cases (aggregate for list-shaped primitives, first non-empty for single-instance) and renders. Verified live on credit-risk (TableView shows risk tier + recommended action + proposed limit; AlertList shows DTI / credit-score warnings; KanbanBoard groups accounts by approval state) and contract-review (SideBySideView shows current clause vs proposed redline; DocumentReader shows clause blocks + annotations) and demand-prediction (TimeSeriesChart with 6-point weekly demand forecast).
- **Click-through from every Operate primitive to the source-case trace.** TableView gets a synthetic Source case column; AlertList / KanbanBoard items wrap as Links; SideBySide / Document / Conversation / Schedule / TimeSeries get a Source case footer caption. All hrefs are checked to start with `/` before render. Per-case `caseHref` threads through the resolver `Carrier`; per-row links on multi-row primitives, footer captions on single-source primitives.
- **demand-prediction workflow seeded by default** (`apps/kernel/scripts/seed_demo.py`). Fixture has existed since A3.1 but was never wired into the seed; the demo now covers three workflow shapes (TableView/AlertList/KanbanBoard for risk, SideBySideView/DocumentReader for review, TimeSeriesChart for forecasting).
- **Inbox filter chips** on `/workspaces/[wsId]/inbox` (`All` / `Proposals` / `Sandbox errors`). URL-driven via `searchParams`, three parallel `listProposals` calls so the sandbox-errors chip degrades to 0 if its fetch flakes without blacking out the rest of the inbox.
- **Overview now shows the workflow's NL description.** First paragraph always visible in a left-bar callout; remaining paragraphs collapse into a `<details>` so the page stays scannable. Previously only available via hover-tooltips on the workflow title.
- **Operate page Inputs section.** Reads triggers (env_generators), data sources, and tools straight from the spec; no agent emission needed.

### Changed
- **Operate is now production execution, not loop diagnostics.** Resolver gains a `context: 'overview' | 'operate'` discriminator. `'operate'` suppresses iteration-meta (MetricCards, TimeSeriesChart of val_score) + eval-prediction primitives (TableView, AlertList, KanbanBoard from per-case bool predictions) — they fall through to a primitive-specific `empty` until the agent emits real domain-shaped output via `output_payload_json`. Operate status pill flips to "Latest output captured" when at least one case carries a payload; otherwise "Awaiting first production run". The Operate-tab Outputs section renders any non-empty payload primitive; otherwise a dashed empty state explaining eval-suite predictions stay on Overview. Operator persona route (`/operator/[workflowId]`) gets the same `context: 'operate'` wiring.
- **Overview tab is meta-only.** Dropped the duplicated spec primitives that mirrored the Operate tab; Overview now shows: workflow description, next-step card (gen-eval-iterate flow), lift chart, iteration list, AgentAnatomy. Operate-shaped primitives (TableView / AlertList / KanbanBoard / etc.) live only on Operate.
- **Default agent model bumped to `claude-sonnet-4-6`** (was `claude-haiku-4-5-20251001`). Haiku consistently drops the `output_payload_json` field even with required schema + imperative system prompt; Sonnet complies reliably. `DEFAULT_MAX_TOKENS` bumped from 1000 → 4000 — the payload runs 1.5-3k tokens and a 1k cap forces the model to silently drop the field to fit.
- **Operate tab fallback when spec has only one tab.** Resolver now cascades `tabs.find(operate) ?? tabs[1] ?? tabs[0]` so single-tab specs (credit-risk's "Portfolio") still render the Operate view instead of going blank.

### Fixed
- **Fly bootstrap: pgvector + build-context.** Two real divergences caught running `make fly-bootstrap` end-to-end. (1) `flyctl postgres create` provisions `flyio/postgres-flex:17.2`, which doesn't ship the pgvector extension, so migration `0001_substrate.sql`'s `CREATE EXTENSION IF NOT EXISTS vector` failed in the kernel `release_command`. Fixed by deploying the upstream `pgvector/pgvector:pg17` image directly as the `ownevo-pg` app (new `infra/postgres-pgvector/` with its own `fly.toml` + README). `OWNEVO_DATABASE_URL` is now composed from a locally cached `POSTGRES_PASSWORD` (gitignored `.fly-pg-password`, chmod 600) + the well-known `ownevo-pg.flycast` host, replacing `flyctl postgres attach` (which only works against unmanaged Fly Postgres). (2) `flyctl deploy --config apps/web/fly.toml --remote-only` from the repo root resolves the build context against cwd, not the config directory — the remote builder uploaded the wrong tree and `COPY package*.json ./` in the web Dockerfile found no lockfile, so `npm ci` failed. Workaround: `cd apps/web && flyctl deploy …` in both the bootstrap script (Step 8) and the `make fly-deploy-web` target. Upstream tracked as `superfly/flyctl#752`. `docs/runbooks/fly-deploy.md` walks both fixes.
- **`Design with agent` button: spinner + ETA during navigation.** Clicking the button on `/workflows/new` pre-fetches the design page server-side (one Sonnet 4.6 call, ~6–12 s) and the button looked frozen until the swap. `router.push()` is now wrapped in `startTransition` so the label flips to a spinner + `Starting design agent — ~10s` while `discoveryPending` is true; `aria-busy` is set for screen readers. The `Generate` button (including its ⌘↵ shortcut) is guarded against firing while the design-agent nav is in flight, so two concurrent LLM calls can't race. Mirrors the existing NL-gen `SubmitButton` pending behaviour.
- **Settings model-catalog: distinguish `DEMO_MODE` 503 from a real kernel failure.** `/api/models` is intentionally blocked under `DEMO_MODE=true`; before this fix the Settings page surfaced the 503 with `Restart the kernel or check logs` copy, which read as a broken kernel on the public demo. The catalog-error state is split into two branches via `KernelApiError.status`: any 503 from `/api/models` renders a normal settings card explaining model selection is disabled on this deployment; any other failure still renders the recoverable-error banner.
- Stale `AgentOS view` copy in the operator shell renamed to `owner view` (AgentOS was retired in v0.8.0; the workspace UI is the owner shell).
- Inbox + operator-shell polish: `wsId` threaded properly through filter-chip hrefs, `aria-current` set on the active chip, defensive `String()` on inputs that landed in template strings.
- Sandbox-errors inbox chip degrades to 0 rather than 500ing the whole inbox when `/api/proposals?state=gate-failed` flakes.
- XSS guard on the operator route's `?ws=` query param (kebab-slug validation before interpolation into hrefs).
- Open-redirect guard on the Inbox proposal links.
...
- **`scripts/doctor.sh` / `make doctor`** — preflight check: tool versions, `.env` contents (handles quoted `ANTHROPIC_API_KEY="…"` form), Fly auth, sandbox-image presence. `--dev` / `--deploy` / `--all` modes, summarized pass/warn/fail counts, exits non-zero when any check fails.
- **`scripts/smoke.sh` / `make smoke` / `make fly-smoke`** — post-deploy smoke test (<10 s, no LLM calls). Hits `/api/health` (status + db fields), `/api/workflows` (≥1 row), `/api/audit?limit=1`. Optional `--web <URL>` arm checks the Next.js app. Safe against `DEMO_MODE=true`.
- **`make help` / `make help-all`** — help text reorganized. Common targets (Getting started / Local stack / Seed / Verify / Fly.io deploy / Docs) with bold TTY-conditional headers go to `make help`; benchmark / dogfooding tier moves to `make help-all`.

### Fixed
- `ANTHROPIC_API_KEY` was exposed in the process argument list when staging kernel secrets; switched to `flyctl secrets import` via stdin.
- `flyctl postgres attach` sets `DATABASE_URL`; the kernel reads `OWNEVO_DATABASE_URL`. Bootstrap now captures the connection string from attach stdout and stages it under the expected name. Partial-run guard: if `DATABASE_URL` is present but `OWNEVO_DATABASE_URL` is missing on re-run, the script exits with actionable instructions.
- `doctor.sh` color formatting: `printf "%b ✓ %s\n" "$C_OK$C_RESET"` concatenated open+close before the symbol, producing uncolored output. Fixed to `printf "%b ✓ %s%b\n" "$C_OK" "$1" "$C_RESET"`.
- `smoke.sh` usage comment referenced `https://demo.ownevo.ai/api` as the API base URL, causing double-path (`/api/api/health`) when passed as the positional arg. Corrected to `https://demo.ownevo.ai`.
- `setup.sh` bootstrapped `.env` with world-readable permissions (644); added `chmod 600` immediately after `cp .env.example .env`.
- `Makefile` `BOLD`/`RESET` variables emitted raw ANSI escape codes in CI / piped output; switched to TTY-conditional `$(shell [ -t 1 ] && printf '...' || true)`.

### Added
- **Every LLM call surface is now parameterizable via `OWNEVO_*_MODEL` env vars**. Override chain per surface: `caller model=` → `OWNEVO_<SURFACE>_MODEL` → hardcoded `DEFAULT_MODEL`. New env vars: `OWNEVO_INSTRUCTION_PROPOSER_MODEL`, `OWNEVO_META_EVAL_MODEL`, `OWNEVO_AGENT_SOLVER_MODEL`, `OWNEVO_CLUSTER_LABEL_MODEL`, `OWNEVO_CLUSTER_JUDGE_MODEL`, `OWNEVO_APPROVER_MODEL`, `OWNEVO_LOOP_MODEL` (`OWNEVO_NL_GEN_MODEL` extended to also fall back at module import). Cloud defaults unchanged; operators opt in by setting the env var. `docker-compose.yml` forwards all eight to the kernel container. Per-surface picks with empirical evidence documented in `docs/local-model-testing.md` § "Cloud model picks per surface" and `docs/ENV_VARS.md` §5.
- **`ANTHROPIC_BASE_URL` honored at every `AsyncAnthropic` construction site** (was only `nl_gen` before). New `apps/kernel/src/ownevo_kernel/api/_anthropic_client.py` factors a shared `build_async_anthropic(api_key)` helper that bakes in the docker-compose empty-string interpolation trap (`${VAR:-}` → empty string → SDK fails with `UnsupportedProtocol`). `workflows.py`'s three sites now use the helper.
- **Retry-on-`ValidationError` for all 4 NL-gen forced-tool generators** (`workflow_spec`, `sim`, `metric`, `eval`). On `ValidationError`, `nl_gen/_validation_retry.py` sends the pydantic errors back as a `tool_result is_error=true` so the model can correct. Default `max_retries=4` (5 attempts total); cloud frontier models pass on attempt 1 (zero cost). Each generator also passes a `normalize` callback that force-overwrites `schema_version` to strip local-LLM training priors that don't yield to in-prompt directives. Empirical lift on `qwen/qwen3.6-35b-a3b` LMS: 15 → 2 validation errors across attempts. `generate_full_pipeline(..., max_retries=N)` threads the override uniformly.
- **Three `.env.*.example` files**: `.env.example` (cloud defaults), `.env.local-lms.example` (route every Anthropic-format surface to LMS), `.env.hybrid.example` (cloud HTTP API + local dev-script loop driver).
- **Per-surface model recommendations** in `docs/local-model-testing.md`. Headline finding from a 6-tool retail-demand E2E test: setting `OWNEVO_AGENT_SOLVER_MODEL=claude-sonnet-4-6` lifted val_score from **0.500 → 1.000** vs the Haiku default. Haiku stays the cheap default for single-tool classifiers; Sonnet is the right pick for multi-tool workflows where Haiku's scattered failures don't cluster well enough for the proposer to grip on.
...
### Added
- **Pre-generation ambiguity scan wired into the discovery chat panel.** Once the static template-aware interview reaches `done=true`, `/workspaces/[wsId]/workflows/new/design` now calls a new `POST /api/design-agent/description-conflicts` and surfaces every rule-based finding as an additional question in the chat. The Generate button stays disabled until each finding is either answered or skipped — the same gating discipline that already applies to the static questions. Answers ride into the existing `discovery_transcript` (kind `ambiguity`, with `question_index` continuing past the static range), so the audit chain + the Audit-tab transcript card both pick them up without a schema change. Closes the 9.1.3 acceptance criterion ("a description like 'maximize recall, zero false positives' produces a forced-choice question before Generate enables"). The post-generation `/ambiguity-report` endpoint stays as-is for the inferred-artifact + metric-direction passes that need a `WorkflowSpec`.
- **`POST /api/design-agent/description-conflicts`** — stateless wrapper around `find_description_conflicts(description)`. Accepts `{description}`, returns `{findings: AmbiguityFinding[]}`. No spec, no `workflow_spec_id`, no LLM call. Lives on the same `design_agent_ambiguity` router as `/ambiguity-report` so the two scans share one route module and one set of HTTP tests. Description ≥ 50 chars; `extra='forbid'` so a stray `spec` field (allowed on `/ambiguity-report`) returns 422 here.
- **Design-agent discovery transcript on the per-workflow Audit tab.** `/workspaces/[wsId]/workflows/[wfId]/audit` now reads `design_agent_log` off `GET /api/workflows/{id}` alongside the existing flat `audit_entries` list and renders the discovery conversation as a grouped, chronological card above the log: one row per question (Q1 / Q2 / …), the kind label (`metric` / `ambiguity` / `trigger` / `surface` / `premise`), the question text, and the operator's answer (or "skipped" in italic when `answer` is null). When the persisted `AmbiguityReport` carries findings, a second card below renders them severity-tagged with their `suggested_question` so a non-engineer reviewer sees the same trade-offs the design agent flagged before generation. The flat audit list still shows every `design-agent-negotiation` + `design-agent-ambiguity` row for hash-chain integrity; the new card is the friendly view. Closes the 9.1.4 acceptance criterion ("per-workflow Audit tab renders the conversation chronologically").
- **`DesignAgentLog` / `DesignAgentLogEntry` / `AmbiguityReport` / `AmbiguityFinding` TypeScript shapes** on `apps/web/lib/api.ts` matching the kernel pydantic models. `WorkflowAnatomy.design_agent_log` is now typed as `DesignAgentLog | null` (was previously absent from the TS surface even though the kernel was already returning it).
...
- **Design-agent ambiguity-detection pass** (`ownevo_kernel.design_agent.ambiguity`). Post-generation scan over a `WorkflowSpec` + optional `MetricDefinition` that surfaces what NL-gen otherwise has to guess. Two passes today: (a) **inferred-artifact scan** — flags every tool / persona / data source / env generator / reviewer with `Provenance.kind == "inferred"`; reviewer findings are high severity, others medium; (b) **description / metric conflict scan** — rule-based detection of canonical contradictions ("maximize recall, zero false positives", "don't change the model"), plus a `metric.direction` vs `spec.success_criterion.direction` cross-check. Every finding carries a `suggested_question` the design agent surfaces verbatim in the discovery conversation. Public API: `analyze_workflow(description, spec, metric_definition=None) -> AmbiguityReport`.
- **`POST /api/design-agent/ambiguity-report`** — stateless endpoint that wraps `analyze_workflow`. Accepts `{description, spec, metric_definition?}` and returns the `AmbiguityReport`. DB-free, key-free, no LLM call. Out-of-shape `spec` or empty description returns 422. The LLM-judge variant for cross-artifact conflicts is its own slice; the endpoint contract stays the same when it lands.
- **Design-with-agent authoring surface** at `/workspaces/[wsId]/workflows/new/design`. Three-pane layout: starting-point summary on the left (description + template), chat panel in the centre (driven by `POST /api/design-agent/next-question`), progress + Generate on the right. Operator answers or skips each discovery question; the Generate button enables when the kernel returns `done=true`. The answered transcript appends to the description as a "Design-agent discovery" appendix before NL-gen so the negotiated metric / ambiguity / trigger / surface / premise choices are visible on the review page. ⌘↵ / Ctrl-↵ inside the answer textarea sends the current draft. Entry point: new "Design with agent" button on `/workflows/new` (next to Cancel and Generate) that routes here with the current `template_id` + `description` as query params. Persistence into a dedicated `design_agent_log` column + audit-chain mirror lands in the next slice.
- **`DiscoveryQuestionKind` expanded from 2 to 5 variants** — `metric`, `ambiguity`, `trigger`, `surface`, `premise`. Locked before `POST /api/design-agent/next-question` freezes the wire format so additional kinds do not require a schema migration. Generic fallback now ships at least one example question per kind; per-template prompt sets stay focused on the verbatim demo-plan beats (metric + ambiguity) and can opt into the additional kinds as the design-agent posture extends. Public runtime tuple `DISCOVERY_QUESTION_KINDS` re-exports the Literal for clients that need to enumerate it.
