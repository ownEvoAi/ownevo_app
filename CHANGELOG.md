...
### Changed
- **Trigger actions are now routed through workspace-scoped RLS connections.** All three trigger action functions (`action_run_clustering`, `action_run_iteration`, `action_ingest_failures`) now accept a `workspace_id` parameter and acquire connections via `acquire_workspace_conn` instead of bare `pool.acquire()`, so every workspace-scoped table write (`traces`, `failure_clusters`, `iterations`) is correctly bound under RLS. `TriggerDispatcher` resolves the workspace by querying the `workflows` row before dispatching; `EmailIngester` and `SlackIngester` fetch the workspace inline before calling `action_ingest_failures`. `action_run_iteration` is now purely fire-and-forget (no longer creates the `iterations` row upfront or returns an iteration ID; the runner owns that row lifecycle), and builds the Anthropic client directly so the background task is fully self-contained.
