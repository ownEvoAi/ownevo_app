...
### Added
- **Audit chain export.** New `GET /api/audit/export` returns the entire audit log as canonical JSON (sorted keys, no whitespace — same bytes the `verify` endpoint reports). Web surface adds an `Export chain` button next to `Verify chain` on `/workspaces/[ws]/audit`; the download is proxied through a Next.js route handler so the kernel URL stays server-side. Closes the "export in an open format" sovereignty claim with a one-click artifact.
- **Per-workflow model routing is now load-bearing.** New `apps/kernel/src/ownevo_kernel/llm/router.py` dispatches a `provider:model` slug from `workflows.agent_model_id` to a live chat client (`AsyncAnthropic` for Anthropic, `AsyncOpenAI` for OpenAI / xAI / Gemini / Fireworks / OpenRouter, `OllamaChatClient` for Ollama daemons). `iteration_runner.run_one_iteration_for_workflow` looks up the workflow's stored slug, builds the client through the router, and threads it into `run_nl_gen_demo_loop` so the agent solver actually swaps providers per workflow row. The instruction proposer continues to use the env-Anthropic client. Boot-time `check_provider_api_keys` warns at startup when an enabled provider has no matching API key. The honesty subnote on the picker (`Runtime routing wiring lands in a follow-up`) is gone — the dropdown now changes behaviour. `.env.example` reorganized to promote OpenRouter as the recommended cross-vendor path and feature Ollama with `qwen3.6-35b-a3b` as the all-local default.
- **`Request changes` decision on the proposal review surface** — middle-ground between Approve and Reject. New `proposal_state = 'changes-requested'`, new audit kind `proposal-changes-requested`, new endpoint `POST /api/proposals/{id}/request-changes` (comment required — the steering text drives the next iteration). The decision lands as an `approvals` row with `decision = 'request-changes'`; no eval case is created (steering is feedback to the loop, not a regression case). On the next iteration for the workflow, `iteration_runner` picks up the latest steering comment and threads it into the loop's `initial_instruction` so the agent + downstream proposer see the redirect. Migration `0015_changes_requested.sql` extends the `proposal_state` + `audit_kind` enums and widens the `approvals.decision` CHECK constraint.
