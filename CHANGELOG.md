...
### Added
- **Pool sizing and per-connection statement_timeout are env-tunable.** The kernel's asyncpg pool now reads three new env vars at boot: `OWNEVO_DB_POOL_MIN_SIZE` (default 1), `OWNEVO_DB_POOL_MAX_SIZE` (default 10), and `OWNEVO_DB_STATEMENT_TIMEOUT_MS` (default 30000). The timeout is applied via the pool's `setup` callback on every acquire because asyncpg's pool issues `DISCARD ALL` on release, which clears any `SET` issued earlier; the callback re-applies the GUC after the reset so the cap holds for every checkout, not just the first one. A query that exceeds the cap is cancelled by Postgres and surfaces as `asyncpg.QueryCanceledError`, so one runaway statement can no longer pin a connection until the upstream client gives up. Setting `OWNEVO_DB_STATEMENT_TIMEOUT_MS=0` opts a process out (useful for long-running maintenance scripts that share the pool); env values are validated at boot so a typo fails loudly there rather than at the first query. The API lifespan now opens its pool through the same `open_pool()` helper that scripts already use, so production and dev paths see identical defaults instead of diverging hard-coded values.
