...
| `OWNEVO_DB_STATEMENT_TIMEOUT_MS` | optional | `30000` | `db.py::open_pool` | Per-connection `statement_timeout` (ms). Applied via the pool's `setup` hook on every acquire, because asyncpg's pool runs `RESET ALL` on release which would otherwise clear the GUC. A query past the cap surfaces as `asyncpg.QueryCanceledError`. Set to `0` to opt out (useful for long-running maintenance scripts that share the pool); not recommended for the API process. |
