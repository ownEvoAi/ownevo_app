...
| `OWNEVO_DB_POOL_MIN_SIZE` | optional | `1` | `db.py::open_pool` | Lower bound on the asyncpg pool. Must be ≥ 1 — a zero floor means every request pays the connect handshake. Validated at boot; a bad value fails fast there rather than at the first query. |
| `OWNEVO_DB_POOL_MAX_SIZE` | optional | `10` | `db.py::open_pool` | Upper bound on the pool. Tuned for a single Next.js dev server; raise for higher-concurrency deploys. `min > max` is rejected at boot. |
| `OWNEVO_DB_STATEMENT_TIMEOUT_MS` | optional | `30000` | `db.py::open_pool` | Per-connection `statement_timeout` (ms). Applied via the pool's `setup` hook on every acquire, because asyncpg's pool issues `DISCARD ALL` on release which would otherwise clear the GUC. A query past the cap surfaces as `asyncpg.QueryCanceledError`. Set to `0` to opt out (useful for long-running maintenance scripts that share the pool); not recommended for the API process. |
