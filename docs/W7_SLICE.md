...
*Drafted 2026-05-08 on `feat/w7-plan`. Source: PLAN.md § Phase 3 W7 (16 rows: 13 in Track 1, 3 in Track 3). Goal of this doc: pick a sub-slice that satisfies the W7 exit gate, defer the rest to W8 without blocking the investor programvideo, and give each PR a clear scope before any code lands.*
...
`workspace_id` doesn't exist (D4 single-tenant). The URL param `[wsId]` is cosmetic for MVP — backend ignores it. Schema-clean retrofit when next deployment lands.

---

## investor-video critical path (the binding constraint)
...
| C | **A+B replay → reproduce the prior-art harness** | Frozen baseline + loop autonomous (= the reference auto-harness) on training subset. Validation gate: condition B reproduces published 0.78 ±5pp. | 7.3.3 | M |
...
W7 weekend  ──  Track 3 slices B + C — the prior-art harness ±5pp gate
```

Each slice = one PR. Six PRs Track 1; three Track 3.

**Single most-load-bearing PR:** slice 2 (Health + LiftChart). Without a credible lift chart, the cold open of the investor programvideo has no visual hook. Build it second so the rest can hang off it. Schema, API, and a visible-in-browser climbing line all need to exist by end of W7 Tuesday.
