...
*Last updated: 2026-05-11 (v3.24 — W8 Track 0 inserted ahead of Track 1: 8 React leaf primitive components + ScheduleGrid schema + workflow-Overview wiring via mock data resolver; layer D real resolver deferred to Phase 2. See [Version log](#version-log) for the full revision history.)*
...
**Track 0 exit gate:** A non-engineer reviewer opens `/workspaces/acme/workflows/m5-demand-prediction` (Docker stack up, M5 seeded) and sees a live-looking forecast workspace — metric tiles, lift chart, SKU-at-risk table, markdown-alert list — instead of the current placeholder banner. The data behind the primitives is hand-curated for now; live wiring is layer D (8.1.x dependent on the agent producing structured render data).

**Deferred to Phase 2 (NOT in W8):**
- **D — Real data resolver.** Agent output → typed `UIPrimitive` data instances flowing through to renderers. Requires either (a) a new "render data" generation step after the agent loop, (b) agents emitting structured outputs alongside traces, or (c) a kernel-side resolver that joins `WorkflowSpec.ui.primitives[].source` against the workflow's iteration outputs. Choice depends on whether multi-tenant retrofit (TODO-1) or τ³ integration lands first. Phase-2 design doc owed.
