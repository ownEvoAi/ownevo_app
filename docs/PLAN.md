...
| 8.4.10 | **TableView end-to-end + first binding (Phase B — operator-shell parity for credit-risk)** | Extend `Primitive.TableView` schema (Pydantic + Zod) with `binding: { source: 'case-output', columns: [{key, label, path, format?}], sort?, limit? }`. Add `TableView` branch to `apps/web/lib/primitive-data-resolver.ts` that fetches `case-outputs?iteration=latest` and maps `columns[].path` over each item. New component `apps/web/app/components/primitives/table-view.tsx` matching mock 30 (sticky header, mini ✓ / ⚠ buttons, "show N more" footer). Add the binding to `credit-risk` spec (one-time edit) + one other workflow's spec for diversity. Resolver falls back to `kind: 'empty'` when no rows → existing "Coming soon" banner still covers cold-start. | `/operator/credit-risk?ws=acme` after one iteration shows a per-applicant recommendation table (id · recommended action · confidence · rationale) sourced from real `iteration_case_outputs` rows, not mock data. Visually parity with `www/preview/s26-rk7p3/30-operator-demand.html` left-column SKU table (column titles + sticky header + show-more footer). The "Coming soon" banner on `/workspaces/acme/workflows/credit-risk/operate` disappears once the iteration's TableView resolves. |
