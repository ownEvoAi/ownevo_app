...
*Last updated: 2026-05-03 (v3.2 — W2.1 kernel-side agent tools shipped (PR #6 + review fixes); Claude Agent SDK middleware adapter deferred as a separate follow-on slice. v3.1 history: W1 sections reconciled with shipped reality after PR #1 (substrate) + PR #2 (W1 days 3-5 runtime). Spike outcome NO-GO recorded; W1.4-W1.7 deliverables match what's in the repo; OTel / Langfuse / `metric.py` / Day-1 LightGBM baseline moved to W2 where they actually depend on the agent-tool surface and Claude Agent SDK middleware. v3 history: applies six CEO-review decisions from MVP v4.3 — D2 audit reframe, D3 local Docker sandbox, D4 single-tenant for MVP, D5 τ³ B-frame, D6 `core/` 2-day spike, D7 NL-gen meta-eval.)*
...
| 2.6 | **M5 baseline pipeline runs end-to-end** | (Deferred from W1.6) `metric.py` implements RMSE + WRMSSE; held-out fold helper defined per Phase 0 decision (last 28 days = test, prior 28 = validation). Then: agent (Claude Agent SDK) writes Day-1 LightGBM baseline (6 skill files: `data_loader.py`, `feature_engineer.py`, `model_trainer.py`, `outlier_handler.py`, `ensemble.py`, `predictor.py`). Pipeline runs in sandbox on held-out fold. Baseline RMSE recorded as the floor. | `pytest -k test_m5_metric` confirms RMSE/WRMSSE on a fixture matches a known reference value within 1e-6. `make m5-baseline` writes 6 skill files (v1) + records baseline RMSE in `iterations` table. RMSE is reproducible across two runs to within numeric tolerance. |
