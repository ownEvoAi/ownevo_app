...
8 weeks to a investor-grade demo on **three pillars**: the natural-language workflow generator (the customer-facing IP), the M5 code-gen-loop benchmark (supply-chain VP credibility), and the τ³-bench head-to-head with NeoSigma (investor / AI-engineer credibility).
...
5. **Workspace strategy post multi-tenant retrofit (2026-05-12).** Today, benchmark workflows (M5 forecasting, tau-bench replays) live in the same `acme` workspace as customer workflows, separated only by a `workflows.kind` column + sidebar partition. Once D4 lands, split into purpose-scoped workspaces: (a) a dedicated `_benchmarks` workspace for kernel validation runs so they never appear in a customer's sidebar, and (b) per-vertical demo workspaces (`demo-legal`, `demo-supply-chain`, `demo-credit-risk`, `demo-clinical`, etc.) seeded with industry-appropriate workflows + eval suites. Each vertical demo doubles as a recruiting / discovery surface and a sales reference customer. The current `kind` column carries forward — production / benchmark / demo — but `workspace_id` becomes the primary partition. Implementation cost: a few hours on top of the multi-tenant work since the substrate already separates concerns; the gating cost is seed data per vertical (~1 day per vertical to author plausible workflow descriptions + eval cases).
...
**What this means honestly:** our static baseline (WRMSSE 1.30) is *below the naive baseline* — by design. The agent needs room to improve, so we deliberately use a minimal baseline. The lift the loop produces (val_score 0.331 → 0.399, +20.5% relative on our val_score metric) is measurable and reproducible, but is not competitive with the M5 leaderboard. **The claim is loop semantics (promote / reject / compound), not absolute M5 performance.** A future Stage D run with `--sandbox-mem-mb 1024` + a stronger starting baseline + more iterations is the path toward absolute-WRMSSE numbers a domain expert would call competitive — but that's not the the application bar; the loop itself is. Cross-iteration failure memory shipped in PR #40 (TODO-23) — Stage D can now run with the loop's memory in-context.
...
**Goal:** by end of Week 8, the workspace UI from `www/preview/yc-s26-rk7p3/` is wired to the live demand-prediction backend, the τ³-bench head-to-head against NeoSigma is published with the human-approval gate engaged, and the demo video + reproducibility artifacts are shipped.
...
| 8.4.9 | **Per-case agent output capture (Phase A — operator-shell parity prerequisite)** | New table `iteration_case_outputs(iteration_id, eval_case_id, output_json, passed, created_at)` via migration `0008_iteration_case_outputs.sql`. Iteration runner (`apps/kernel/src/ownevo_kernel/iteration_runner.py` + the agent solver path in `middleware/claude_sdk/runner.py`) requires the agent's final turn to call a `submit_case_output` tool whose JSONSchema is declared in `WorkflowSpec.output_schema`; the runner INSERTs one row per case with the tool args + bool `passed` from the existing gate logic. Gate scoring unchanged (still `passed`). New endpoint `GET /api/workflows/{wfId}/case-outputs?iteration=latest|<idx>` returns `{iteration_index, items: [{eval_case_id, input, output_json, expected_behavior, passed}]}`. Unblocks 8.4.10 + the TableView / AlertList bullet in 8.4.6. | Running an iteration on `credit-risk` writes one `iteration_case_outputs` row per eval case with the agent's structured output (e.g. `{recommended_action, confidence, rationale}`). `GET /api/workflows/credit-risk/case-outputs?iteration=latest` returns the rows. Existing regression gate still scores the bool and passes/fails iterations identically to pre-change. |
| 8.4.10 | DONE 2026-05-12 — **TableView end-to-end (Phase B)**. | First-cut: web-side only — kept Pydantic + Zod TableView schema unchanged to skip a freeze-test churn. `apps/web/lib/primitive-data-resolver.ts` gained a `TableView` branch that resolves to the latest `case-outputs` rows: 5 columns (case_id · predicted · expected · result pill · rationale), failed-first ordering. `apps/web/lib/api.ts` got `CaseOutputList` + `getWorkflowCaseOutputs`. Operator shell + workspace Operate tab + workspace Overview tab all pass `caseOutputs` into the resolver and render `<TableView>` on the new `kind: 'TableView'` branch. Existing `TableView` component reused as-is plus two new pill tones (`pass` → green, `fail` → red). | `/operator/credit-risk?ws=acme` after iteration #10 shows 12 per-case rows (9 fail, 3 pass) with case_id + predicted/expected ticks + result pill + agent rationale; failed-first ordering verified in rendered HTML. Same TableView appears on `/workspaces/acme/workflows/credit-risk/operate` and the Overview tab. The spec-level `binding: { source, columns: [{path,...}] }` contract — and a true per-workflow column mapping that reads from `output_json` paths instead of hardcoded keys — is deferred to a follow-up once an agent emits richer `submit_case_output` shapes (today: `predicted` / `expected` / `rationale` are all we get). |
...
**Sequencing note:** 8.4.1 + 8.4.2 ship together (rip + seed are inseparable). 8.4.3 → 8.4.4 → 8.4.5 are strict order. 8.4.6 can land any time after 8.4.5. **8.4.7 + 8.4.8 shipped together in PR #85** (`feat/real-ui-loop`) — alongside lifecycle controls (workflow delete, description edit, deploy/rollback, eval-case manual curation), Health polish, the Operate sub-tab, the operator shell, the Connect on-ramp, workspace Traces, and per-case rationale plumbing. **All eight v3.26 lifecycle + polish TODOs closed in the same PR; their numbers were reused for new items.** **8.4.9 is a hard prerequisite for the TableView / AlertList / KanbanBoard bullet of 8.4.6** — Phase A captures structured per-case output, 8.4.10 wires the first primitive (TableView) end-to-end, and subsequent primitives are additive (one commit each, same template). Remaining gaps (per-cluster reasoning summary, stale-running sweep, new-workflow review step, etc.) are captured in [TODOS.md](../TODOS.md).

**Tradeoff vs Track 1 demo materials:** Track 1 rows (8.1.1 demo video, 8.1.4 website screenshots) currently assume the hand-curated demo data shipped in Track 0. After Track 4 lands, video / screenshots show real product state — more credible, but requires running real iterations against seeded workflows ahead of recording. The seed script + 8.4.5 iteration trigger make that loop cheap. Track 1 timing depends on Track 4 reaching 8.4.6 (or accepting empty-state Overview in the video).

#### Track 1 — Demo materials (M5 + τ³ + NL-gen together)

| # | Deliverable | Files / location | Validation |
|---|---|---|---|
| 8.1.1 | **Record 90-second demo video** | Per North Star storyboard. Live demand-prediction workspace (rendered via Track 0 primitives — hand-curated demo data acceptable for video) + real M5 results from W6 + real τ³ split-screen from W8 + live NL-gen flow scene. | Single take or minimal cuts; reviewer who watches understands all 3 pillars without a slide. |
...
| **Phase 2 W6 (Tracks A + B)** | Live NL-gen demo in <5 min for external reviewer + M5 30-day replay meets all thresholds | live demo loses one or both pillars |
...
| **Phase 3 W8 (final)** | demo video shipped + both results posts + reproducibility + τ³ ≥+35% threshold | MVP slips to 9-10 weeks |
...
- **Next cycle priorities:** TODO-42 (Fly.io live demo, unblocks investor call signal), TODO-43 (demo video record), TODO-44 (README opener polish).
...
- These items are independent of the W8 demo bar; tracking them outside W8 keeps the demo critical path lean.
...
- **W8 exit criteria updated** to include Track 4 gate alongside demo video / benchmark posts / reproducibility / τ³ threshold.
