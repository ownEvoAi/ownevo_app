...
*Last updated: 2026-05-12 (v3.26 — W8 Track 4 rows 8.4.7 + 8.4.8 added: iteration runner populates audit_entries / failure_clusters / traces, and a per-iteration drill-down surfaces case-by-case predictions. Closes the three dead-end tabs (Failures / Traces / per-workflow Audit) that the UI exposes but the runner doesn't currently populate. See [Version log](#version-log) for the full revision history; deferred lifecycle + polish items moved to [TODOS.md](../TODOS.md) TODO-36 → TODO-43.)*
...
| 8.4.7 | **Iteration runner populates audit_entries + failure_clusters + traces** | `apps/kernel/src/ownevo_kernel/iteration_runner.py` currently writes only `iterations`, `skills`, `skill_versions`, `proposals`. Three workflow tabs (Failures / Traces / Audit) are wired into the UI but stay dead-ends for NL-gen workflows because the runner doesn't persist the data they read. This row writes: (a) `audit_entries` rows for `iteration-started` + `gate-run-completed` + `skill-version-created`, (b) one `failure_clusters` row per non-trivial cluster the loop already computes (`run_nl_gen_demo_loop` returns a `ClusteringResult` — persist via the existing `B3.x` cluster_writer path), (c) one `traces` row per per-case agent call (event_id, trace_id, role, content). | Running an iteration on an NL-gen'd workflow produces ≥1 cluster row visible in the Failures tab, ≥1 trace row per eval case visible in the Traces tab, and `iteration-started` + `gate-run-completed` audit entries visible in the workflow-scoped Audit tab. None of those tabs show "no data" after a real iteration. |
| 8.4.8 | **Per-iteration drill-down: case-level predictions** | The lift chart shows `val_score: 0.333` but the user can't see *which 8 of 12 cases failed* or what the agent predicted vs expected. New route `apps/web/app/workspaces/[wsId]/workflows/[wfId]/iterations/[iterationId]/page.tsx` — table of every eval case run during that iteration with columns: case_id · expected_value · agent_prediction · pass/fail · trace link. New kernel endpoint `GET /api/iterations/{id}/cases` (joins `eval_cases` + iteration's per-case predictions stored alongside the trace). Lift-chart points + iteration table rows link to this page. | Clicking iter #2 on credit-risk's lift chart lands on a table showing 12 cases with prediction vs expected; failed rows highlighted; each row links to the per-case trace from 8.4.7. The "improvement loop" pitch goes from a magic number to an explainable signal. |
...
**Sequencing note:** 8.4.1 + 8.4.2 ship together (rip + seed are inseparable — landing 8.4.1 alone makes the dev UI uselessly empty for visual work). 8.4.3 → 8.4.4 → 8.4.5 are strict order. 8.4.6 can land any time after 8.4.5; until it does, primitives stay in honest empty state on workflows that have not been iterated. **8.4.7 + 8.4.8 follow 8.4.5** — they extend the iteration runner's write path and surface the per-iteration drill-down; both ship together so the new Traces/Failures rows have a UI to land on. **Lifecycle controls** (workflow delete, description edit, eval-case manual curation, deploy/rollback button) and the **Tier-4 polish list** (Health primary-workflow heuristic, sidebar truncation, in-flight indicator, MetricCards delta copy) are tracked in [TODOS.md](../TODOS.md) TODO-36 → TODO-43 — not in W8 because they're independent of the YC demo bar.
