...
| 2.6 | **M5 baseline pipeline runs end-to-end** (PR #4 metric foundation ✅; PR #11a scaffolding + seasonal-naive Day-1 ✅; PR #11b LightGBM bodies + sandbox image + reproducibility CI in flight) | (Deferred from W1.6) `apps/kernel/src/ownevo_kernel/datasets/m5_metric.py` implements RMSE + WRMSSE per the M5 paper; `make_held_out_fold(catalog, val_days=28, test_days=28)` carves the train / val / test split per Phase 0's lock; refuses zero-scale series. `apps/kernel/src/ownevo_kernel/benchmark/` — `BenchmarkRunner` Protocol + `BenchmarkResult` + `SyntheticBenchmarkRunner` for the W2.2a self-test. **Shipped in PR #11a:** `M5BenchmarkRunner` over `(catalog, fold, pipeline_fn)` with per-series reward `exp(-RMSSE_i)`; 6 SKILL_FORMAT-compliant skill files at `apps/kernel/baselines/m5_lightgbm/skill_v1/` (`data_loader`, `outlier_handler`, `feature_engineer`, `model_trainer`, `predictor`, `ensemble`); in-process orchestrator; `scripts/m5_baseline.py` + `make m5-baseline` end-to-end; DB-write path registers skills idempotently (skips re-registration when body is unchanged) and appends an `iterations` row at `MAX(iteration_index)+1`. **v1 skill bodies = seasonal-naive** (last 28 train days repeated as the 28-day forecast — defensible "no model" Day-1 floor); kernel stays pandas/lightgbm-free (deps live in `baselines/`, dependency-injected via `M5PipelineFn` Protocol). **Pending in PR #11b:** real LightGBM bodies (`lightgbm`/`pandas`/`sklearn` pinned in a sandbox Dockerfile); flip the runner to drive `LocalDockerSandbox` via `run_pipeline`; reproducibility CI with the cache strategy from B3.4. The agent (W4) iterates the same 6-file split — replacing seasonal-naive with LightGBM is its first real diff once the SDK middleware adapter lands. | `pytest -k test_m5_metric` confirms RMSE/WRMSSE on a fixture matches a known reference value within 1e-6. ✅. `pytest tests/test_benchmark_m5.py tests/test_baselines_m5_lightgbm.py` covers runner Protocol conformance + per-series reward formula + orchestrator determinism (17 tests, no DB/Docker required) ✅ (PR #11a). `make m5-baseline` writes 6 skill files (v1) + records baseline RMSE in `iterations` table ✅ (PR #11a; DB path skipped when `OWNEVO_DATABASE_URL` is unset). RMSE is reproducible across two runs to within numeric tolerance ✅ (`test_run_baseline_is_deterministic` asserts bit-identical predictions). |
