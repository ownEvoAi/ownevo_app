...
*Last updated: 2026-05-12 (v3.25 — W8 Track 4 added: rip mock infrastructure (mocks.ts / primitives-mock-data.ts / WORKFLOW_MOCKS), seed real workflow rows via `make seed-demo`, and close the gen → eval → propose loop in the UI (live `POST /api/nl-gen/generate`, eval-cases page, "Run iteration" button). Layer-D real resolver promoted from "Phase 2 deferred" to in-W8 row 8.4.6. See [Version log](#version-log) for the full revision history.)*
...
**Sequencing note:** 8.4.1 + 8.4.2 ship together (rip + seed are inseparable — landing 8.4.1 alone makes the dev UI uselessly empty for visual work). 8.4.3 → 8.4.4 → 8.4.5 are strict order. 8.4.6 can land any time after 8.4.5; until it does, primitives stay in honest empty state on workflows that have not been iterated.
