...
*Last updated: 2026-05-03 (v3.3 — W2 spine reconciled: W2.3 eval-cases + W2.4 audit log shipped (PR #3); W2.6 metric.py + held-out fold helper + BenchmarkRunner Protocol shipped (PR #4); W2.2 core run_gate function + W2.2a self-test harness shipped (PR #8 in flight) — DB-write wrapper that creates iterations + proposals + audit rows on each gate run is a separate follow-on slice that lands alongside the M5 LightGBM baseline pipeline; TODO-17 sandbox runner-exit-code hardening shipped (PR #9 in flight). v3.2 history: W2.1 kernel-side agent tools shipped (PR #6); Claude Agent SDK middleware adapter deferred. v3.1 history: W1 sections reconciled with shipped reality after PR #1 + PR #2; spike outcome NO-GO recorded; W1.4-W1.7 deliverables match what's in the repo. v3 history: applies six CEO-review decisions from MVP v4.3 — D2 audit reframe, D3 local Docker sandbox, D4 single-tenant for MVP, D5 τ³ B-frame, D6 `core/` 2-day spike, D7 NL-gen meta-eval.)*
...
| 2.6 | **M5 baseline pipeline runs end-to-end** (partial: metric.py + held-out fold + BenchmarkRunner Protocol ✅ PR #4; LightGBM baseline writer + DB-write wrapper pending) | (Deferred from W1.6) `apps/kernel/src/ownevo_kernel/datasets/m5_metric.py` implements RMSE + WRMSSE per the M5 paper; `make_held_out_fold(catalog, val_days=28, test_days=28)` carves the train / val / test split per Phase 0's lock; refuses zero-scale series. `apps/kernel/src/ownevo_kernel/benchmark/` — `BenchmarkRunner` Protocol + `BenchmarkResult` + `SyntheticBenchmarkRunner` for the W2.2a self-test. **Pending in this slice:** agent (Claude Agent SDK) writes Day-1 LightGBM baseline (6 skill files: `data_loader.py`, `feature_engineer.py`, `model_trainer.py`, `outlier_handler.py`, `ensemble.py`, `predictor.py`); pipeline runs in sandbox on held-out fold; M5BenchmarkRunner implements the Protocol; gate's DB-write wrapper records baseline RMSE in `iterations` table. | `pytest -k test_m5_metric` confirms RMSE/WRMSSE on a fixture matches a known reference value within 1e-6. ✅. `make m5-baseline` writes 6 skill files (v1) + records baseline RMSE in `iterations` table. RMSE is reproducible across two runs to within numeric tolerance. |
