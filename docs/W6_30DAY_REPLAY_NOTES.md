...
| Run | Parent baseline | Loop driver | Best ever val_score | Lift on val_score | Type of insight |
|---|---|---|---|---|---|
| v6 | skill_v1 (3 features, default LightGBM, regression loss) | Sonnet 4.6 | 0.4077 | **+23.2%** | re-discovers textbook ML |
| v7 | skill_v2 (Tweedie loss, 20 features, tuned hyperparams — PR #72) | Sonnet 4.6 | 0.4247 | **+0.62%** | hyperparameter tuning only |
| **v8 (in flight, snapshot iter 2/30)** | skill_v2 (same as v7) | **Opus 4.7** | **0.4339 so far** | **+2.79% so far** | **cross-skill interaction reasoning** |
...
- *Defensive claim:* "On both weak and strong baselines, the gate stopped every regression." v6 + v7 + v8 (so far): zero false promotions across 65+ paid iters.
