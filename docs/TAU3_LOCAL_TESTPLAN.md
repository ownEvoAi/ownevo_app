...
**Session totals (2026-05-10/12):** 13 attempts, **2 end-to-end val_score landings**:
...
- Run 14: `unsloth_qwen36_full_local_64k` (2026-05-12) — **SANDBOX_ERROR, but reward 0.77 (N=39)**. Single long-tail task 101 hit 4hr per-task wall (initial 44 min + R1 70 min → 14400s timeout). 39/40 clean signals **cross-quant generalizability**: unsloth ≈ qwen/ within noise band (0.77 vs 0.75 winner). Cycle wall-time ~16h43m total (mostly task 101 retry loop).
- Run 15 (in flight, 2026-05-12): `qwen3coder_30b_lms_full_local_64k` — LMS variant of prior session's +14.9% Ollama lift driver. Tests whether LMS path with KV cache fixes the retail-task-weakness seen on Ollama (where it scored 0.15 avg).
