...
| zai-org/glm-4.7-flash (LMS) | — | — | ⚠ ¹⁵ | — | ¹⁵ kickoff message exceeded model context **at default LMS load** (retail system prompt ~5228 tokens > model default ctx). **Fix confirmed:** `lms load zai-org/glm-4.7-flash -c 32768` (phase3_full_lms_sweep already does this). Also available on Ollama as `glm-4.7-flash:latest` (19.0GB). **Planned as Run A: all-3-roles, LMS openai, ctx=32768.** GLM-4.7 uses DeepSeek-2 architecture (30B, 18.13GB) — thinking suppression behavior on LMS unknown. If LMS strips thinking (as it does for qwen3.6 family), may perform well. |
...
Other attempts (in chronological order):
- Run 9: `granite_full_local_64k` (all-3 granite-8B on LMS) — ERROR, granite-8B too weak to write_skill in 4 iters.
- Run 10: `qwen36loop_graniteagent_64k_smoke` (mixed: qwen3.6 loop + granite-8B task/user) — KILLED at 4/40, throughput-bound (~11hr ETA), reward 0.50.
- Run 11: `qwen36loop_graniteagent_64k_retry` — KILLED before phase 2 (user pivot to granite-30B).
- Run 13: `granite_30b_full_local_64k` — SANDBOX_ERROR. Granite-30B emitted write_skill (better than 8B!) but `agent.py` lacked the required `HarnessAgent` class → 40/40 blocked at sandbox import. Codegen quality too low.
- Run 14: `unsloth_qwen36_full_local_64k` (2026-05-11) — **SANDBOX_ERROR, but reward 0.77 (N=39)**. Single long-tail task 101 hit 4hr per-task wall (initial 44 min + R1 70 min → 14400s timeout). 39/40 clean signals **cross-quant generalizability**: unsloth ≈ qwen/ within noise band (0.77 vs 0.75 winner). Cycle wall-time ~16h43m total (mostly task 101 retry loop).
- Run 15: `qwen3coder_30b_lms_full_local_64k` (2026-05-11) — **PASS val_score=0.1250**. LMS path fixed the throughput issue (~48s/task vs Ollama's stuck-task pattern) but retail reward stayed weak (0.13). Confirms qwen3-coder structurally retail-weak.
- Run 16: `qwen36loop_qwen35_4b_ollama_smoke` (2026-05-11) — **SANDBOX_ERROR**. Mixed: qwen3.6-35b-a3b LMS loop + `ollama_chat/qwen3.5:4B` task/user. 0/40 evaluated, 40 infra_errors. Root cause: LiteLLM `ollama_chat` adapter sends malformed tool-calls payload for qwen3.5 family → Ollama HTTP 415 "Unsupported Media Type" after 4 retries. ~14 min wall-time.
- Run 17: `qwen36loop_qwen35_9b_ollama_smoke` (2026-05-11) — **SANDBOX_ERROR**. Same mixed topology with `ollama_chat/qwen3.5:9B`. Identical 415 failure, 0/40 evaluated, ~5 min. Confirms failure is model-size independent. **`ollama_chat/qwen3.5:*` track CLOSED** — deterministic LiteLLM adapter bug, not a model-quality issue.
- Run 18: `qwen36loop_qwen35_9b_lms_anthropic_smoke` (2026-05-11) — **SANDBOX_ERROR**. Mixed: qwen3.6-35b-a3b LMS loop + `anthropic/qwen/qwen3.5-9b` LMS task/user. 8/40 evaluated, 32 infra_errors. Root cause: LMS default jinja template for qwen3.5-9b fails on multi-turn retail conversations ("No user query found in messages"). Loop completed end_turn (5 iters, v_seq=156). **Transport (anthropic/ routing) confirmed working** — no 415, no timeout. Fix applied 2026-05-11: user applied **froggeric v13** template to both qwen/qwen3.5-9b AND qwen/qwen3.5-4b in LMS UI. Retry queued (Run 20 / #32). ~7 min wall-time.
- Run 19: `gemma4_31b_full_local_64k` (2026-05-11→12) — **SANDBOX_ERROR**. google/gemma-4-31b all-3-roles on LMS, ctx=65536. 36/40 evaluated, 4 infra_errors (tasks 55, 56, 60, 61). Root cause: LMS HTTP 500 "Failed to resolve model metadata for google/gemma-4-31b." — intermittent LMS registry failure under sustained load (>2hr, many concurrent contexts). Avg reward **0.62** (N=36); peaked at 0.73 mid-run. Loop: 7 iters, 6 tool_calls, 0 tool_errors, v_seq=158. Duration ~2h32m. Key finding: dense 31B avoids the max_tokens cap (unlike MoE gemma-4-26b-a4b); retail reward 0.62 > e4b (0.18) but < qwen3.6 (0.75). Infra failure is LMS registry instability, NOT model quality — could retry with `openai/google/gemma-4-31b` alias fix if LiteLLM metadata mapping issue identified.
- Run 20: `qwen36loop_qwen35_9b_lms_anthropic_smoke_v2` (2026-05-12) — **SANDBOX_ERROR**. qwen3.6-35b-a3b loop + `anthropic/qwen/qwen3.5-9b` task/user. 0/40 evaluated, 40 infra. **v13 template CONFIRMED working** — no jinja errors at all. Root cause: qwen3.6 proposal (v_seq=161) referenced `self.known_facts` (uninitialized attribute in `HarnessAgent`) → AttributeError on every task. This is a PROPOSAL CODEGEN quality issue, not a qwen3.5-9b task agent issue. Loop: 7 iters, 7 tool_calls, 2 tool_errors. ~4 min wall-time. Key finding: qwen3.5-9b with v13 template is viable as task agent — the qwen3.6 proposer introduced the bug.
- Run 21: `qwen36loop_qwen35_4b_lms_anthropic_smoke` (2026-05-12) — **PASS val_score=0.8250. New all-time record (+10pp over Run 8 winner 0.7500).** qwen3.6-35b-a3b loop + `anthropic/qwen/qwen3.5-4b` task/user on LMS (/v1/messages), froggeric v13 template. 40/40 evaluated, 0 infra_errors. Loop: 5 iters, 5 tool_calls, 2 tool_errors, v_seq=163. ~24 min wall-time. Tokens: 33292 in / 6801 out. Key finding: mixed topology (large proposer + small task agent) outperforms the all-same-model baseline. qwen3.5-4B at 4B params scores 10pp higher than qwen3.6-35B as task agent — smaller/faster model is BETTER here. v13 template is confirmed production-viable for both 4B and 9B.
- Run 22: `qwen36loop_qwen35_9b_lms_anthropic_smoke_v3` (2026-05-12) — **PASS val_score=0.7250.** qwen3.6-35b-a3b loop + `anthropic/qwen/qwen3.5-9b` task/user, froggeric v13 template. 40/40 evaluated, 0 infra_errors. Loop: 4 iters, 4 tool_calls, 1 tool_error, v_seq=165. ~24 min wall-time. **Key finding: 9B (0.7250) < 4B (0.8250) by −10pp.** Smaller task agent is definitively BETTER. Confirmed inverse scaling on retail: 4B > 9B > 35B for task agent capability. Killed after cycle 1 (cycle 2 started, stopped to free resources for next runs).
- **Run 24 (scale-up):** `qwen36_all3_real_scaleup_5c` (2026-05-12T07:20Z → 09:33Z, 2h13m) — **5-cycle scale-up on REAL winner config** (proposer `qwen/qwen3.6-35b-a3b` LMS openai + task/user `anthropic/qwen/qwen3.6-35b-a3b` LMS /v1/messages w/ v13 template). Cycles: **0.7500 / 0.6750 / 0.7250 / 0.8250 🎯 / 0.7000**. Mean 0.7350. Cycle 4 hit the same 0.8250 ceiling as Run 21 via a different skill (proposal `917d8d89`: lookup_tracker + STOP at 8 tool calls; vs Run 21's `33f6e90d`: known_facts cross-turn memory). **Two distinct skill patterns reach 0.8250 — the ceiling is real, set by qwen3.6-35b-a3b's task-agent capability, not skill quality.** Confirms that the v13-template + anthropic-routing path (originally credited to a 4B model under JIT-fallback) is the actual lift driver.
- **Run 25 (task-agent test #6):** `qwen36_loop_gptoss20b_task_smoke` (2026-05-12T~09:30Z, ~22 min) — **PASS val_score=0.3000.** qwen3.6-35b-a3b LMS proposer + `openai/openai/gpt-oss-20b` LMS task/user. 40/40 evaluated, 0 infra_errors. 52pp below qwen3.6-35b-a3b baseline (0.75). **Weaker task agent does NOT lift via skill** — confirms task-agent quality is the ceiling.
- **Run 26 (task-agent test #5):** `qwen36_loop_devstral2_task_smoke` (2026-05-12T03:30Z) — **SANDBOX_ERROR.** qwen3.6-35b-a3b LMS proposer + `openai/mistralai/devstral-small-2-2512` LMS task/user. 40/40 infra_errors. Root cause: LMS jinja template error — "After the optional system message, conversation roles must alternate user and assistant roles except for tool calls and results." Devstral's bundled chat template doesn't tolerate tau3's tool-call/result turn structure. Loop ran clean (5 iters, 5 tool_calls, 1 tool_error, v_seq=192, proposal `22152c06`) — failure is template-side, not proposer or codegen. **Deferred:** needs froggeric-style template override in LMS UI before retry.
- **Run 27:** `qwen36_loop_qwen35_9b_task_smoke` (2026-05-12T16:14Z → 17:03Z, ~49 min) — **SANDBOX_ERROR.** qwen3.6-35b-a3b LMS proposer + `anthropic/qwen/qwen3.5-9b` LMS task/user (v13 template, **ctx=32768**). 37/40 evaluated, 3 infra_errors — tasks 36, 100 (and 1 more) hit "Context size has been exceeded" on `/v1/messages`. Same long-tail issue qwen3.6-35b-a3b had at ctx=32k. **Partial data (N=31): avg reward 0.6129** (19/12 ones/zeros) — well below Run 22's claimed 0.7250 for "9B". **JIT-fallback hypothesis CONFIRMED:** Run 22's task agent was actually qwen3.6-35b-a3b via fallback (real 9B is genuinely weaker). Proposal v_seq=194, `9ae40d65`. Loop clean (4 iters, end_turn). Fix: reload at ctx=65536.
- **Run 28:** `qwen36_loop_qwen35_9b_task_ctx65k` (2026-05-12T17:09Z → 17:31Z, ~22 min) — **PASS val_score=0.5750.** Run 27 retry with `lms load qwen/qwen3.5-9b -c 65536`. 40/40 evaluated, 0 infra_errors. Loop: 5 iters, 5 tool_calls, 1 tool_error, v_seq=196, `280130ad`. **Key result — JIT-fallback DEFINITIVELY CONFIRMED:** Real qwen3.5-9b at **0.5750** vs Run 22's claimed **0.7250** for "9B" — 15pp gap = Run 22 served qwen3.6-35b-a3b via JIT-fallback, not real 9B. Real retail τ³ task-agent ranking: **qwen3.6-35b-a3b (0.75) > qwen3.5-9b (0.575) > gpt-oss-20b (0.30) ≫ qwen3.5-4b (~0.30)**. "Inverse scaling 4B > 9B > 35B" claim fully invalidated; bigger > smaller for retail task agent. Log: `log/tau3_p2/qwen36_loop_qwen35_9b_task_ctx65k_p2_cycle1.log`.
- **Run 29 launch blocker:** task #8 `qwen/qwen3-30b-a3b-2507` LMS load fails silently 3×: `(X) CAUSE  (Exit code: null). Please check settings and try loading the model again.` Persists after `lms server stop && start`. Other models load fine (verified by reloading 35b-a3b post-each-attempt). Falling back to **non-2507 `qwen/qwen3-30b-a3b`** (same MoE arch, different release) per user direction.
