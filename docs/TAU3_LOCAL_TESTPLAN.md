...
**Session totals (2026-05-10/12):** 21 attempts, **4 end-to-end val_score landings**:
...
- Run 21: `qwen36loop_qwen35_4b_lms_anthropic_smoke` — **PASS val_score=0.8250** (qwen3.6 loop + qwen3.5-4b task/user, v13 template). 40/40 clean, ~24 min. **New all-time record (+10pp over Run 8).** Smaller task agent beats the all-roles winner.
...
- Run 22 (in-flight): `qwen36loop_qwen35_9b_lms_anthropic_smoke_v3` — same topology as Run 21 but with 9B task/user instead of 4B. Hypothesis: does larger task agent beat 0.8250? or does 4B's faster inference + lower latency advantage outweigh 9B capability?
