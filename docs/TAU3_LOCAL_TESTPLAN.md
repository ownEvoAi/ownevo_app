...
**Session totals (2026-05-10/12):** 20 attempts, **3 end-to-end val_score landings**:
- Run 8: `qwen36lms_ctx65k_smoke` — **PASS val_score=0.7500** (LMS qwen3.6-35b-a3b all-3, ctx=65k, v12 template). 40/40 clean, ~27 min. **Best landing.**
...
- Run 15: `qwen3coder_30b_lms_full_local_64k` — **PASS val_score=0.1250** (LMS qwen/qwen3-coder-30b all-3, ctx=65k). 40/40 clean, ~30 min. Third landing; confirms qwen3-coder retail-weak regardless of backend (was 0.15 on Ollama, now 0.13 on LMS).
...
- Run 21 (queued): `qwen36loop_qwen35_4b_lms_anthropic_smoke` — new test: qwen3.6-35b-a3b loop + anthropic/qwen3.5-4b task/user on LMS. v13 template applied. 4B is faster; may have better concurrency than 9B.
