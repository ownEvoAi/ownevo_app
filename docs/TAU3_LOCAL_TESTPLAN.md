...
| `ollama_chat/granite4.1:8b` (Ollama, ctx=32768, NUM_PARALLEL=1) **as task agent in mixed run** | ⚠ throughput + accuracy-bound | **2026-05-12 Run 38 `qwen36_loop_granite8b_ollama_task_smoke`** (killed at 4/40 @ 0.50, ~15 min): **UTF-8 surrogate bug ABSENT** — Ollama's Modelfile chat template avoids the bytes LMS bundled jinja produced. Confirms the LMS surrogate bug is template-specific, not model-intrinsic. But per-task wall-time ~6-7 min (dense 8B + thinking preamble + NUM_PARALLEL=1 + both task/user-sim are granite), ETA ~2hr per cycle. 2/4 tasks hit `Simulation terminated prematurely. too_many_errors` — granite4.1:8b degrades at conv depth ≥10. **Verdict: infra-clean, ~0.30-0.50 ceiling, too slow on Ollama.** Prefer MoE Ollama variants (qwen3:30b-a3b) over dense thinking ones for task-agent role. |
