...
| **M9** | Extend or write `scripts/run_tau3_loop.py`. Loop agent reads current skill + recent failure clusters via `analyze_failures`, proposes new agent.py via `write_skill`, gate runs sandboxed `SandboxedTauBenchRunner` against test split. Mirrors `run_improvement_loop.py` shape. | M — ~1 day | One end-to-end loop iteration on tau3 produces a valid `iterations` row + skill_versions row at seq=2 |
