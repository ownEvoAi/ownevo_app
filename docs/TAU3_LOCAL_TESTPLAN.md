...
| `anthropic/qwen/qwen3.5-9b` (LMS, v13 template) | ⚠ INDETERMINATE → retest in-flight | **Run 22 reported PASS val_score=0.7250** with JIT enabled. `qwen/qwen3.5-9b` IS a valid LMS identifier, so JIT may have auto-loaded real 9B OR fallen back to loaded qwen3.6-35b-a3b. **Run 27 in-flight (2026-05-12T16:14Z, ETA 60-90 min)** with JIT disabled + explicit prior load resolves this. |
...
- **Run 27 (task-agent test #4, in-flight):** `qwen36_loop_qwen35_9b_task_smoke` (2026-05-12T16:14Z, ETA 60-90 min) — qwen3.6-35b-a3b LMS proposer + `anthropic/qwen/qwen3.5-9b` LMS task/user (v13 template), 4hr per-task timeout. **First retest of real qwen3.5-9b** with JIT disabled and explicit prior load — resolves the ambiguity in Run 22's PASS (was it real 9B or JIT-fallback to 35B?).
