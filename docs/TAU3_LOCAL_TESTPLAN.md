...
| qwen3.6:27b (Ollama) | ✗ ³ᵉ | — | n/a | n/a | ³ᵉ **Run 23, 2026-05-12T02:35Z**: `httpx.ReadTimeout` after ~915s wall-time. **Root cause: qwen3.6:27b is a 27B DENSE model** (17.4GB on disk). First Ollama request required ~5 min disk load + slow dense-model generation (3-5 tok/s for a dense 27B vs ~30 tok/s for MoE 35b-a3b which has only 3B active params). Wall time exceeded the 600s httpx timeout in `ollama_native.py`. **Fix:** `DEFAULT_TIMEOUT_SECONDS` bumped `600s → 1800s` (run 23v2 in-flight with the fix). The MoE vs Dense distinction is load-bearing for timeout budgeting — all prior Ollama successes used the 35b-a3b MoE variant, not dense models. |
| qwen3.5-9b | — | ✗ ⁴ | ✗ ⁴ | ✅ ⁴ | ⁴ F14g — 0/3 via OpenAI, 3/3 via Anthropic. API-format-load-bearing. **2026-05-11 tau3-retail mixed smokes**: `ollama_chat/qwen3.5:4B` and `ollama_chat/qwen3.5:9B` BOTH fail with `litellm.APIConnectionError "Unsupported Media Type"` (HTTP 415 from Ollama after 4 retries) → 40/40 infra → SANDBOX_ERROR. Deterministic and model-size independent; **`ollama_chat/qwen3.5:*` track CLOSED** pending upstream LiteLLM ollama_chat adapter fix for qwen3.5 tool-calls payload shape. LMS-anthropic (`anthropic/qwen/qwen3.5-9b`) path retry in flight as of 2026-05-11. |
...
**Session totals (2026-05-10/12):** 23 attempts, **5 end-to-end val_score landings**:
