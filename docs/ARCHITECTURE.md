...
## 2. The improvement loop — the core IP
...
- `eval_cases` are the durable IP — they accumulate over time and define the regression suite. They are what makes the "every prior fix is regression-tested" claim load-bearing.
...
There is **no `workspace_id` column on any domain table** today. The `[wsId]` URL slug is cosmetic. Multi-tenant retrofit is `TODO-1`, a bounded 1-2 week job before customer #2, and the schema was designed retrofit-friendly. Do not add patterns that would fight a future `workspace_id` column.
...
**ownEvo platform primitives** (internal IP):
...
- **τ³-bench condition C + NeoSigma reproduction** (PLAN W7.3.3 + 8.3.x).
