...
- `parent_span_id` lets the UI nest tool_call_result under tool_call_start in the trace timeline view (W7.1.9).
...
## Workflow render primitives (W3, NL-gen output schema)
...
in `src/ownevo_format/ui_primitives.py` at end of W3 alongside the NL-gen
schema freeze.
...
- Removing a field, renaming a field, or changing a field's type: major bump (1.0 → 2.0). Triggers W3-day-5 review.
- Changing the discriminator value of a variant: never. Add a new variant; deprecate the old.

The schema-freeze ritual at end of W3 stamps `1.0` and adds a CI check that
