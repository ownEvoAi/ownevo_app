...
- **Status (2026-05-09):** Dry-run shipped on `dryrun/w6-preview-nlgen-demo`. Two consecutive runs against `demand-prediction` (haiku 4.5 agent, Sonnet 4.6 proposer, 3 cycles): run 1 = 34.2 s wall, lift `[0.20, 0.80, 0.60]` (+0.40, regression on cycle 2, `is_climbing=False`); run 2 = 17.2 s wall, lift `[0.20, 1.00, 1.00]` (+0.80, `is_climbing=True`, matches storyboard exactly). Total dry-run wall (page render + 2 loop runs) was under 2 minutes; the **5-minute reviewer budget holds with margin**. Four UX gaps surfaced and recorded in `docs/W6_PREVIEW_DRYRUN.md`: (§1) storyboard pointed at the legacy `/workflows/preview` URL — fixed in this branch; (§2) `Run baseline ›` button tooltip referenced a non-existent `POST /api/nl-gen/generate` — tooltip rewritten to point at the CLI demo path in this branch; (§3) cycle-2 regression risk on a live take — recommendation is to use `--cycles 2` for the YC video; (§4) no live progress output during the loop — separate follow-up. Run logs and rendered preview HTML preserved at `docs/W6_PREVIEW_DRYRUN_artifacts/`.
